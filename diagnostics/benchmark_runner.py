...
from diagnostics.benchmark_checks import (
    benchmark_async_gather_tasks,
    benchmark_command_tree,
    benchmark_discord_roundtrip,
    benchmark_event_loop_yield,
    benchmark_gateway_latency,
    benchmark_guild_level_config,
    benchmark_handler_specs,
    benchmark_health_checks,
    benchmark_level_config_count,
    benchmark_locale_file_load,
    benchmark_localization,
    benchmark_manifest_compare,
    benchmark_manifest_json_parse,
    benchmark_parallel_select_one,
    benchmark_pool_acquire,
    benchmark_prefix_commands,
    benchmark_select_one,
)
...
class BenchmarkRunner:
    ...
    async def run_all(self) -> BenchmarkSummary:
        ...
        for phase_fn in (
            self._run_phase_a_discord,
            self._run_phase_b_database,
            self._run_phase_c_localization_io,
            self._run_phase_d_commands,
            self._run_phase_e_health,
            self._run_phase_f_handlers,
            self._run_phase_g_runtime,
        ):
            try:
                await phase_fn()
    ...
    async def _run_phase_b_database(self) -> None:
        ...
        phase.results.extend(
            [
                await benchmark_pool_acquire(self.bot),
                await benchmark_select_one(self.bot),
                await benchmark_parallel_select_one(self.bot),
                await benchmark_level_config_count(self.bot),
                await benchmark_guild_level_config(self.bot, self._guild_id),
            ]
        )
    ...
    async def _run_phase_d_commands(self) -> None:
        ...
        phase.results.extend(
            [
                benchmark_command_tree(self.bot),
                benchmark_manifest_compare(self.bot),
                benchmark_prefix_commands(self.bot),
            ]
        )
    ...
    async def _run_phase_g_runtime(self) -> None:
        ...
        phase.results.extend(
            [
                await benchmark_event_loop_yield(),
                await benchmark_async_gather_tasks(),
            ]
        )
    ...
    async def _finalize(self) -> None:
        ...
        summary_body = "\n".join(
            [
                ...
                "See thread for per-phase timings.",
            ]
        )
        if s.error_count:
