...
async def setWelcomeChannel(  # type: ignore[no-any-unimported]
    ...
    await set_welcome_channel(commandInfo.guild.id, channel.id, message, imgUrl)  # type: ignore[arg-type,union-attr]
...
async def welcomeNewUser(member: discord.Member) -> None:  # type: ignore[no-any-unimported]
    welcomeChannel = await get_welcome_channel(member.guild.id)
    if welcomeChannel is None:
        return

    background_frames, _ = await get_image_or_gif_frames(welcomeChannel[3])  # type: ignore[func-returns-value,misc]
