...
async def removeCountingChannel(commandInfo: commandInfo, channel: discord.TextChannel):
    ...
    if not commandInfo.user.guild_permissions.moderate_members:
        embed = tanjunEmbed(
            title=tanjunLocalizer.localize(commandInfo.locale, "minigames.removecountingchannel.error.no_moderate_members_perms.title"),
            description=tanjunLocalizer.localize(commandInfo.locale, "minigames.removecountingchannel.error.no_moderate_members_perms.description"),
        )
