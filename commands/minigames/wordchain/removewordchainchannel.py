...
async def removewordchainchannel(commandInfo: commandInfo, channel: discord.TextChannel):
    ...
    if not commandInfo.user.guild_permissions.moderate_members:
        embed = tanjunEmbed(
            title=tanjunLocalizer.localize(commandInfo.locale, "minigames.removewordchainchannel.error.no_moderate_members_perms.title"),
            description=tanjunLocalizer.localize(commandInfo.locale, "minigames.removewordchainchannel.error.no_moderate_members_perms.description"),
        )
    ...
    if current_progress is None:
        embed = tanjunEmbed(
            ...
            description=tanjunLocalizer.localize(commandInfo.locale, "minigames.removewordchainchannel.error.not_counting_channel.description"),
        )
        await commandInfo.reply(embed=embed)
        return
    ...
    embed = tanjunEmbed(
        ...
        description=tanjunLocalizer.localize(commandInfo.locale, "minigames.removewordchainchannel.success.description", channel=channel.mention),
    )
    await commandInfo.reply(embed=embed)
    ...
    info_embed = tanjunEmbed(
        ...
        description=tanjunLocalizer.localize(commandInfo.locale, "minigames.removewordchainchannel.channel_message.description"),
    )
