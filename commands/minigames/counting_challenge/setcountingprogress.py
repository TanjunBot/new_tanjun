...
async def setCountingProgress(command_info: CommandInfo, channel: discord.TextChannel, progress: int) -> None:
    ...
    embed = tanjunEmbed(title=_NS.success.title(str(command_info.locale)), description=_NS.success.description(command_info.locale).format(channel=channel.mention, progress=progress))
