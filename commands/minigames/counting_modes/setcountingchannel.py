...
async def setCountingChannel(command_info: CommandInfo, channel: discord.TextChannel) -> None:
    ...
    if await require_bot_permissions(command_info, channel):
        return
    await _repo.set_mode_progress(channel_id=channel.id, guild_id=command_info.guild.id, progress=1, mode=CountingMode.DOUBLE, goal=128, counter_id='nobody')
    introduction_embed = tanjunEmbed(title=_NS.modesintroduction.title(str(command_info.locale)), description=_NS.modesintroduction.description(command_info.locale))
