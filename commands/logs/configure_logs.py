...
async def configure_logs(commandInfo: utility.commandInfo):
    ...
    async def build_log_settings_embed(
        locale: str, guild: discord.Guild, selectedIndex: int
    ):
        description = ""
        for index, option in enumerate(LOG_OPTIONS):
            localizedOption = tanjunLocalizer.localize(
                locale, f"commands.logs.configureLogs.configurationEmbed.{option}"
            )
            enabled = log_enabled[index + 1]
            enabledLocalized = (
                tanjunLocalizer.localize(
                    locale, "commands.logs.configureLogs.configurationEmbed.activated"
                )
                if enabled
                else tanjunLocalizer.localize(
                    locale, "commands.logs.configureLogs.configurationEmbed.deactivated"
                )
            )
    ...
    if not log_enabled:
        embed = utility.tanjunEmbed(
            title=tanjunLocalizer.localize(
                commandInfo.locale, "commands.logs.configureLogs.noLogEnabled.title"
            ),
            description=tanjunLocalizer.localize(
                commandInfo.locale,
    ...
    class logConfigureView(discord.ui.View):
        ...
        async def regenerate_embed(self, interaction: discord.Interaction):
            description = await build_log_settings_embed(
                self.locale, self.guild, self.selectedIndex
            )
            self.embed = utility.tanjunEmbed(
                title=tanjunLocalizer.localize(
                    self.locale, "commands.logs.configureLogs.title"
                ),
                description=description,
            )
            self.children[0].disabled = (
                log_enabled[self.selectedIndex + 1] == 1 if log_enabled else True
            )  # Activate button
            self.children[3].disabled = (
                log_enabled[self.selectedIndex + 1] == 0 if log_enabled else True
            )  # Deactivate button
    ...
    configurationEmbed = await build_log_settings_embed(
        commandInfo.locale, commandInfo.guild, 0
    )
    view = logConfigureView(commandInfo.locale, commandInfo.guild, 0)

    embed = utility.tanjunEmbed(
        title=tanjunLocalizer.localize(
            commandInfo.locale, "commands.logs.configureLogs.title"
        ),
