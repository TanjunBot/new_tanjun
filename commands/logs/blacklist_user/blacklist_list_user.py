from api import get_log_user_blacklist as get_log_blacklist_users_api, remove_log_user_blacklist as remove_log_blacklist_user_api, add_log_user_blacklist as add_log_blacklist_user_api
import utility
import discord
...
async def blacklist_list_user(commandInfo: utility.commandInfo):
    ...
    class BlacklistView(discord.ui.View):
        ...
        async def update_view(self, interaction: discord.Interaction):
            if not self.users or len(self.users) == 0:
                ...
                description = "\n".join([f"{'➤' if i == self.selectedIndex else ''} <@{user[0]}>" for i, user in enumerate(self.users)])
            embed = utility.tanjunEmbed(
    ...
    view.add_item(discord.ui.UserSelect(custom_id="user_select", placeholder=tanjunLocalizer.localize(commandInfo.locale, "commands.logs.blacklistListUser.addUser.placeholder")))
