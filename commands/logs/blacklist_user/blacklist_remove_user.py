from api import remove_log_user_blacklist as remove_log_blacklist_user_api, is_log_user_blacklisted as is_log_user_blacklisted_api
import utility
import discord
...
async def blacklist_remove_user(commandInfo: utility.commandInfo, user: discord.Member):
    ...
    if not isBlacklisted:
        embed = utility.tanjunEmbed(
            title=tanjunLocalizer.localize(commandInfo.locale, "commands.logs.blacklistRemoveUser.notBlacklisted.title"),
            description=tanjunLocalizer.localize(commandInfo.locale, "commands.logs.blacklistRemoveUser.notBlacklisted.description")
        )
    else:
        ...
        embed = utility.tanjunEmbed(
            ...
            description=tanjunLocalizer.localize(commandInfo.locale, "commands.logs.blacklistRemoveUser.success.description")
        )
