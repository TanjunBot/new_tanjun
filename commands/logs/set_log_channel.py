from api import set_log_channel as set_log_channel_api, get_log_channel as get_log_channel_api
import utility
import discord
...
async def set_log_channel(commandInfo: utility.commandInfo, channel: discord.TextChannel):
    ...
    if not permissions.send_messages:
        embed = utility.tanjunEmbed(
            ...
            description=tanjunLocalizer.localize(commandInfo.locale, "commands.logs.setLogChannel.botMissingPermission.description")
        )
        await commandInfo.reply(embed=embed)
        return
    ...
    embed = utility.tanjunEmbed(
        ...
        description=tanjunLocalizer.localize(commandInfo.locale, "commands.logs.setLogChannel.success.description", channel=channel.mention)
    )
    await commandInfo.reply(embed=embed)