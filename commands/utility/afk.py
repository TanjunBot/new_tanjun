...
async def afk(command_info: CommandInfo, reason: str) -> None:
    if await check_if_opted_out(command_info.user.id):
        embed = tanjunEmbed(title=locale.commands.utility.afk.opted_out.title(str(command_info.locale)), description=locale.commands.utility.afk.opted_out.description(command_info.locale))
        await command_info.reply(embed=embed)
        return
    if await afk_service.is_afk(command_info.user.id):
        embed = tanjunEmbed(title=locale.commands.utility.afk.already_afk.title(str(command_info.locale)), description=locale.commands.utility.afk.already_afk.description(command_info.locale))
        await command_info.reply(embed=embed)
        return
    await afk_service.set_afk(command_info.user.id, reason)
    embed = tanjunEmbed(title=locale.commands.utility.afk.success.title(str(command_info.locale)), description=locale.commands.utility.afk.success.description(command_info.locale, reason=reason))
    await command_info.reply(embed=embed)
