...
import discord
from api import check_if_reporter_is_blocked, get_report_channel, report_user, accept_report, reject_report, block_reporter
...
async def report(commandInfo: commandInfo, reason: str, user: discord.Member):
    ...
    view.add_item(discord.ui.Button(label=accept_locale, style=discord.ButtonStyle.success, custom_id=f"report_accept;{report_id};{commandInfo.user.id}"))
    reject_locale = tanjunLocalizer.localize(guild_locale, "commands.utility.report.reject.label")
    view.add_item(discord.ui.Button(label=reject_locale, style=discord.ButtonStyle.danger, custom_id=f"report_reject;{report_id};{commandInfo.user.id}"))
    block_reporter_locale = tanjunLocalizer.localize(guild_locale, "commands.utility.report.block_reporter.label")
    view.add_item(discord.ui.Button(label=block_reporter_locale, style=discord.ButtonStyle.danger, custom_id=f"report_block_reporter;{report_id};{commandInfo.user.id}"))
...
async def report_btn_click(interaction: discord.Interaction, custom_id: str):
    ...
    if report_action == "report_accept":
        ...
        embed = tanjunEmbed(
            ...
            description=tanjunLocalizer.localize(
                interaction.locale, "commands.utility.report.report_accepted.description"
            ),
        )
        ...
        embed = tanjunEmbed(
            ...
            description=tanjunLocalizer.localize(interaction.locale, "commands.utility.report.report_rejected.description"),
        )
        await interaction.response.send_message(embed=embed)
        ...
        embed = tanjunEmbed(
            ...
            description=tanjunLocalizer.localize(interaction.locale, "commands.utility.report.reporter_blocked.description"),
        )
        await interaction.response.send_message(embed=embed)
