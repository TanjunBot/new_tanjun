...
async def club(command_info: command_info, club_tag: str):
    ...
    base_description = ''
    base_description += locale.commands.utility.brawlstars.club.description.overview(command_info.locale, name=club_name, trophies=addThousandsSeparator(trophies), description=club_description, required_trophies=addThousandsSeparator(required_trophies))
    pages = []
    for i, member in enumerate(members):
        description = base_description
        description += '\n'
        description += locale.commands.utility.brawlstars.club.description.member(command_info.locale, name=member.name, tag=member.tag, trophies=addThousandsSeparator(member.trophies), role=member.role)
        embed = tanjunEmbed(title=locale.commands.utility.brawlstars.club.title(command_info.locale, name=club_name, tag=club_tag, role=member.role, current_page=i + 1, total_pages=len(members)), description=description)
        pages.append(embed)
    ...
    class SearchModal(discord.ui.Modal):
        ...
        def __init__(self, command_info: command_info):
            super().__init__(title=locale.commands.utility.brawlstars.club.search.title(command_info.locale))
            self.command_info = command_info
            self.add_item(discord.ui.TextInput(label=locale.commands.utility.brawlstars.club.search.label(command_info.locale), placeholder=locale.commands.utility.brawlstars.club.search.placeholder(command_info.locale), required=True))
    ...
    if len(pages) > 1:
        ...
        embed = tanjunEmbed(title=locale.commands.utility.brawlstars.club.titleNoMembers(command_info.locale, name=club_name, tag=club_tag), description=pages[0].description)
