...
async def detail_image(commandInfo: utility.CommandInfo, image: discord.Attachment):  # type: ignore[no-untyped-def]
    ...
    image = image.filter(ImageFilter.DETAIL())  # type: ignore[attr-defined]

    buffer = BytesIO()
    image.save(buffer, format="png")  # type: ignore[call-arg, unused-coroutine]
    buffer.seek(0)
    embed = utility.tanjunEmbed(
        title=tanjunLocalizer.localize(str(commandInfo.locale), "commands.image.detail.success.title"),
        description=tanjunLocalizer.localize(str(commandInfo.locale), "commands.image.detail.success.description"),
    )
