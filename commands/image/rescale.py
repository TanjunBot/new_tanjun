...
async def rescale(commandInfo: utility.commandInfo, image: discord.Attachment, factor: float):
    ...
    buffer.seek(0)
    embed = utility.tanjunEmbed(
        title = tanjunLocalizer.localize(commandInfo.locale, "commands.image.rescale.success.title"),
        description = tanjunLocalizer.localize(commandInfo.locale, "commands.image.rescale.success.description"),
    )
