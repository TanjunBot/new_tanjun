...
async def emboss(commandInfo: utility.commandInfo, image: discord.Attachment):
    ...
    buffer.seek(0)
    embed = utility.tanjunEmbed(
        title = tanjunLocalizer.localize(commandInfo.locale, "commands.image.emboss.success.title"),
        description = tanjunLocalizer.localize(commandInfo.locale, "commands.image.emboss.success.description"),
    )
