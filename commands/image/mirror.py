...
async def mirror(
    commandInfo: utility.commandInfo, image: discord.Attachment, axis: str
):
    ...
    if axis == "x":
        ...
        embed = utility.tanjunEmbed(
            title=tanjunLocalizer.localize(
                commandInfo.locale, "commands.image.mirror.invalidaxis.title"
            ),
            description=tanjunLocalizer.localize(
                commandInfo.locale, "commands.image.mirror.invalidaxis.description"
            ),
        )
        await commandInfo.reply(embed=embed)
        return
