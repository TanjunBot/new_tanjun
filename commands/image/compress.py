...
async def compress(command_info: CommandInfo, image: discord.Attachment, quality: int) -> None:
    ...
    embed = utility.tanjunEmbed(title=locale.commands.image.compress.success.title(str(command_info.locale)), description=locale.commands.image.compress.success.description(str(command_info.locale), newSize=f'{new_size_kb}', oldSize=f'{orig_size_kb}'))
