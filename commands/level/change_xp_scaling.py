...
class PaginationView(View):
    ...
    async def prev_button(self, interaction: discord.Interaction, button: Button[Any]) -> None:  # type: ignore[misc]
        self.current_page = max(0, self.current_page - 1)
        await self.update_message(interaction)

    @discord.ui.button(label="▶️", style=discord.ButtonStyle.gray)
    async def next_button(self, interaction: discord.Interaction, button: Button[Any]) -> None:  # type: ignore[misc]
...
async def show_xp_scalings(commandInfo: CommandInfo, start_level: int = 1, end_level: int = 5) -> None:
    ...
    if total_pages == 1:
        ...
        view.message = message  # type: ignore[attr-defined]
