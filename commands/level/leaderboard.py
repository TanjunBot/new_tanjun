...
async def leaderboard(commandInfo: utility.commandInfo, page: int = 1):
    ...
    if not leaderboard:
        await commandInfo.message.channel.send(tanjunLocalizer.localize(commandInfo.locale, "commands.level.leaderboard.no_data"))
        return
    ...
    def generateButtons(page: int, guild: discord.Guild):
        buttons = []
        if page > 1:
            btn = discord.ui.Button(label=tanjunLocalizer.localize(commandInfo.locale, "commands.level.leaderboard.previous"), style=discord.ButtonStyle.primary, custom_id=f"leaderboard_{page - 1}")
            btn.callback = leaderboardButton
            buttons.append(btn)
        if page < int(len(leaderboard) / 10 + 1):
            btn = discord.ui.Button(label=tanjunLocalizer.localize(commandInfo.locale, "commands.level.leaderboard.next"), style=discord.ButtonStyle.primary, custom_id=f"leaderboard_{page + 1}")
            btn.callback = leaderboardButton
            buttons.append(btn)
        return buttons

    async def leaderboardButton(button: discord.ui.Button, interaction: discord.Interaction):
        ...
        embed = generateLeaderboardEmbed(page, interaction.guild, scaling, custom_formula)
    ...
    def generateLeaderboardEmbed(page: int, guild: discord.Guild, scaling: str, custom_formula):
        ...
        for i in range(10):
            try:
                ...
                break
        print(description)
        embed = utility.tanjunEmbed(title=tanjunLocalizer.localize(commandInfo.locale, "commands.level.leaderboard.title", page = page), description=description)
        return embed
