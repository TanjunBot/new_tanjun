...
async def give_xp_command(command_info: CommandInfo, user: discord.Member, amount: int) -> None:
    if isinstance(command_info.user, discord.Member) and isinstance(command_info.channel, discord.abc.GuildChannel) and (not command_info.channel.permissions_for(command_info.user).manage_guild):
        embed = tanjunEmbed(title=locale.commands.level.givexp.error.no_permission.title(str(command_info.locale)), description=locale.commands.level.givexp.error.no_permission.description(str(command_info.locale)))
        await command_info.reply(embed=embed)
        return
    if amount <= 0:
        embed = tanjunEmbed(title=locale.commands.level.givexp.error.invalid_amount.title(str(command_info.locale)), description=locale.commands.level.givexp.error.invalid_amount.description(str(command_info.locale)))
        await command_info.reply(embed=embed)
        return
    ...
    embed = tanjunEmbed(title=locale.commands.level.givexp.success.title(str(command_info.locale)), description=locale.commands.level.givexp.success.description(str(command_info.locale), user=user.mention, amount=amount, new_xp=new_xp, old_level=old_level, new_level=new_level))
    await command_info.reply(embed=embed)