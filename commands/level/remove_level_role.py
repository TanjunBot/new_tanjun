...
async def remove_level_role_command(commandInfo: commandInfo, role: discord.Role, level: int):
    if not commandInfo.user.guild_permissions.manage_roles:
        embed = tanjunEmbed(
            title=tanjunLocalizer.localize(
                commandInfo.locale, "commands.level.removelevelrole.error.no_permission.title"
            ),
            description=tanjunLocalizer.localize(
                commandInfo.locale, "commands.level.removelevelrole.error.no_permission.description"
            ),
        )
    ...
    if str(role.id) not in existing_roles:
        embed = tanjunEmbed(
            title=tanjunLocalizer.localize(
                commandInfo.locale, "commands.level.removelevelrole.error.role_not_found.title"
            ),
            description=tanjunLocalizer.localize(
                commandInfo.locale, "commands.level.removelevelrole.error.role_not_found.description",
                level=level,
                role=role.mention
            ),
        )
    ...
    embed = tanjunEmbed(
        ...
        description=tanjunLocalizer.localize(
            commandInfo.locale, "commands.level.removelevelrole.success.description",
            role=role.mention,
            level=level
        ),
    )
