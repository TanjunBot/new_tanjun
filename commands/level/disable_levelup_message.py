...
async def disable_levelup_message(commandInfo: commandInfo):
    ...
    if not checkIfHasPro(commandInfo.guild.id):
        embed = tanjunEmbed(
            title=tanjunLocalizer.localize(
                commandInfo.locale, "commands.level.disablelevelupmessage.error.no_pro.title"
            ),
            description=tanjunLocalizer.localize(
                commandInfo.locale, "commands.level.disablelevelupmessage.error.no_pro.description"
            ),
        )
        await commandInfo.reply(embed=embed)
        return
