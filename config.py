...
class Settings(BaseSettings, cli_parse_args=False):
    ...
    """
    model_config: ClassVar[SettingsConfigDict] = SettingsConfigDict(env_file='.env', env_file_encoding='utf-8', extra='ignore', case_sensitive=False, populate_by_name=True)
    token: SecretStr
    ...
    database_port: int = Field(default=3306, validation_alias=AliasChoices('database_port', 'MARIADB_PORT', 'MYSQL_PORT'))
    database_password: SecretStr = Field(validation_alias=AliasChoices('database_password', 'MARIADB_PASSWORD', 'MYSQL_PASSWORD'))
    database_user: str = Field(validation_alias=AliasChoices('database_user', 'MARIADB_USER', 'MYSQL_USER'))
    database_schema: str = Field(validation_alias=AliasChoices('database_schema', 'MARIADB_DATABASE', 'MYSQL_DATABASE'))
...
_REQUIRED_ATTRS: tuple[str, ...] = ('token', 'application_id', 'database_ip', 'database_password', 'database_user', 'database_schema', 'prefix')
_missing = [name for name in _REQUIRED_ATTRS if not getattr(settings, name, None)]
if _missing:
    print(f"FATAL: Missing required setting(s): {', '.join(_missing)}", file=sys.stderr)
    print('Please check your .env file or set these environment variables.', file=sys.stderr)
    sys.exit(1)
