...
from api import get_log_enable, get_log_channel
...
class LogsCommands(discord.app_commands.Group):
    ...
    @app_commands.describe(
        channel="The channel you want set as log channel"
    )
    async def set_log_channel_cmd(self, ctx: discord.Interaction, channel: discord.TextChannel):
    ...
    @app_commands.command(
        name="configure_logs",
        description="Configure the logs"
    )
...
class LogsCog(commands.Cog):
    ...
    async def on_automod_rule_create(self, rule: discord.AutoModRule):
        ...
        # Basic info
        description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.created_by", creator=rule.creator.mention))
        description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.enabled", enabled=("✅" if rule.enabled else "❌")))
        ...
        description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.triggerType", 
        ...
        if rule.trigger.keyword_filter:
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.keywordFilters", keywordFilters=filters))

        # Regex patterns
        if rule.trigger.regex_patterns:
            patterns = "\n".join(f'- {regex}' for regex in rule.trigger.regex_patterns)
            description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.regexPatterns", regexPatterns=patterns))

        # Presets
        description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.presets",
        ...
        if rule.trigger.mention_limit:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.max_mentions", 
        ...
        if rule.exempt_roles:
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.excluded_roles", 
                excluded_roles=roles or "-"))

        if rule.exempt_channels:
            channels = "\n".join(f'- {excluded.mention}' for excluded in rule.exempt_channels)
            description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.excluded_channels", 
        ...
        if len(rule.actions) > 0:
            ...
            for r in rule.actions:
                if(r.type == discord.AutoModRuleActionType.block_message):
                    ...
                    description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.send_warning_message",
                        channel=r.channel_id))

                elif(r.type == discord.AutoModRuleActionType.timeout):
                    description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.timeout",
                        duration=str(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.timeout_duration." + str(r.duration)))))

                elif(r.type == discord.AutoModRuleActionType.block_member_interactions):
                    description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.block_member_interaction",
                        duration=str(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.timeout_duration." + str(r.duration)))))
    ...
    async def on_automod_action(self, execution: discord.AutoModAction):
        ...
        description_parts.append(tanjunLocalizer.localize(locale, "logs.automodAction.actionWasTaken", user=execution.member.mention, channel=execution.channel.mention))
        # Actions
        ...
        if(execution.action.type == discord.AutoModRuleActionType.block_message):
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.timeout",
                duration=str(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.timeout_duration." + str(execution.action.duration)))))

        elif(execution.action.type == discord.AutoModRuleActionType.block_member_interactions):
            description_parts.append(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.block_member_interaction",
                duration=str(tanjunLocalizer.localize(locale, "logs.automodRuleCreate.timeout_duration." + str(execution.action.duration)))))

        description_parts.append(tanjunLocalizer.localize(locale, "logs.automodAction.message", message=(execution.action.content[0:1000] + "..." if len(execution.action.content) > 1000 else execution.action.content)))
    ...
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel):
        ...
        description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelDelete.type", type=str(tanjunLocalizer.localize(locale, "logs.guildChannelDelete.types." + str(channel.type)))))
        description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelDelete.created_at", created_at=utility.date_time_to_timestamp(channel.created_at)))
        if channel.category:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelDelete.category", category=channel.category))
        ...
        if len(channel.overwrites.keys()) > 0:
            ...
            for target, overwrite in channel.overwrites.items():
                ...
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelDelete.permissionOverwriteTarget", target=target_str))
                if allowed:
                    description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelDelete.permissionOverwriteAllowed", permissions=", ".join(allowed)))
                if denied:
                    description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelDelete.permissionOverwriteDenied", permissions=", ".join(denied)))
    ...
    async def on_guild_channel_create(self, channel: discord.abc.GuildChannel):
        ...
        if channel.category:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelCreate.category", category=channel.category))
    ...
    async def on_guild_channel_update(self, before: discord.abc.GuildChannel, after: discord.abc.GuildChannel):
        ...
        if before.name != after.name:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.name", before=before.name, after=after.name))
        
        if before.type != after.type:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.type", before=str(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.types." + str(before.type))), after=str(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.types." + str(after.type)))))
        if before.category != after.category:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.category", before=before.category, after=after.category))

        if before.topic != after.topic:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.topic", before=before.topic, after=after.topic))
        ...
        if before.overwrites != after.overwrites:
            ...
            for target in before.overwrites:
                if target not in after.overwrites:
                    target_str = target.mention if hasattr(target, 'mention') else target.name
                    description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteRemoved", target=target_str))
            ...
            for target, new_overwrite in after.overwrites.items():
                ...
                if old_overwrite is None:
                    ...
                    description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteNew", target=target_str))
                    if allowed:
                        description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteAllowed", permissions=", ".join(allowed)))
                    if denied:
                        description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteDenied", permissions=", ".join(denied)))
                    if neutral:
                        description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteNeutral", permissions=", ".join(neutral)))
                else:
                    ...
                    if any([added_allow, added_deny, added_neutral, removed_allow, removed_deny, removed_neutral]):
                        description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteModified", target=target_str))
                        if added_allow:
                            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteAddedAllow", permissions=", ".join(added_allow)))
                        if added_deny:
                            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteAddedDeny", permissions=", ".join(added_deny)))
                        if added_neutral:
                            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteAddedNeutral", permissions=", ".join(added_neutral)))
                        if removed_allow:
                            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteRemovedAllow", permissions=", ".join(removed_allow)))
                        if removed_deny:
                            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteRemovedDeny", permissions=", ".join(removed_deny)))
                        if removed_neutral:
                            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.permissionOverwriteRemovedNeutral", permissions=", ".join(removed_neutral)))
        ...
        if hasattr(after, "default_auto_archive_duration") and after.default_auto_archive_duration != before.default_auto_archive_duration:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.defaultAutoArchiveDuration", before=before.default_auto_archive_duration, after=after.default_auto_archive_duration))
        ...
        if hasattr(after, "default_thread_auto_archive_duration") and after.default_thread_auto_archive_duration != before.default_thread_auto_archive_duration:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.defaultThreadAutoArchiveDuration", before=before.default_thread_auto_archive_duration, after=after.default_thread_auto_archive_duration))

        if hasattr(after, "nsfw") and after.nsfw != before.nsfw:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.nsfw", before=str(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.yes")) if before.nsfw else str(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.no")), after=str(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.yes")) if after.nsfw else str(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.no"))))

        if hasattr(after, "slowmode_delay") and after.slowmode_delay != before.slowmode_delay:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildChannelUpdate.slowmodeDelay", before=before.slowmode_delay, after=after.slowmode_delay))
    ...
    async def on_guild_update(self, before: discord.Guild, after: discord.Guild):
        ...
        if before.afk_channel != after.afk_channel:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.afkChannel", before=before.afk_channel.mention if before.afk_channel else keinerLocale, after=after.afk_channel.mention if after.afk_channel else keinerLocale))

        if before.afk_timeout != after.afk_timeout:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.afkTimeout", before=before.afk_timeout, after=after.afk_timeout))

        if before.banner != after.banner:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.banner", before=before.banner if before.banner else keinerLocale, after=after.banner if after.banner else keinerLocale))
        ...
        if before.default_notifications != after.default_notifications:
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.defaultNotifications", before=allMembersLocale if before.default_notifications else only_mentions, after=allMembersLocale if after.default_notifications else only_mentions))

        if before.description != after.description:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.description", before=before.description, after=after.description))

        if before.discovery_splash != after.discovery_splash:
            urlLocale = tanjunLocalizer.localize(locale, "logs.guildUpdate.discoverySplashLocales.url")
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.discoverySplash", before=("[" + urlLocale + "](" + before.discovery_splash.url + ")" if before.discovery_splash else keinerLocale), after=("[" + urlLocale + "](" + after.discovery_splash.url + ")" if after.discovery_splash else keinerLocale)))

        if before.emoji_limit != after.emoji_limit:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.emojiLimit", before=before.emoji_limit, after=after.emoji_limit))
        ...
        if removed_emojis:
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.removedEmojis", removed_emojis=removed_list))
        ...
        if before.explicit_content_filter != after.explicit_content_filter:
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.explicitContentFilter", before=disabled if before.explicit_content_filter.disabled else noRole if before.explicit_content_filter.no_role else allMembers, after=disabled if after.explicit_content_filter.disabled else noRole if after.explicit_content_filter.no_role else allMembers))
        ...
        if added_features:
            added_list = "\n".join(f"- {tanjunLocalizer.localize(locale, 'logs.guildUpdate.featuresLocales.' + feature)}" for feature in added_features)
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.addedFeatures", added_features=added_list))

        if removed_features:
            removed_list = "\n".join(f"- {tanjunLocalizer.localize(locale, 'logs.guildUpdate.featuresLocales.' + feature)}" for feature in removed_features)
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.removedFeatures", removed_features=removed_list))
        ...
        if before.icon != after.icon:
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.icon", before=("[" + urlLocale + "](" + before.icon + ")" if before.icon else noIconLocale), after=("[" + urlLocale + "](" + after.icon + ")" if after.icon else noIconLocale)))

        if before.filesize_limit != after.filesize_limit:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.filesizeLimit", before=before.filesize_limit, after=after.filesize_limit))
        ...
        if before.invites_paused_until != after.invites_paused_until:
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.invitesPausedUntil", before="<t:" + str(utility.date_time_to_timestamp(before.invites_paused_until)) + ":R>" if before.invites_paused_until else notPausedLocale, after="<t:" + str(utility.date_time_to_timestamp(after.invites_paused_until)) + ":R>" if after.invites_paused_until else notPausedLocale))

        if before.max_members != after.max_members:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.maxMembers", before=before.max_members if before.max_members else "0", after=after.max_members if after.max_members else "0"))

        if before.max_presences != after.max_presences:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.maxPresences", before=before.max_presences if before.max_presences else keinerLocale, after=after.max_presences if after.max_presences else keinerLocale))

        if before.max_video_channel_users != after.max_video_channel_users:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.maxVideoChannelUsers", before=before.max_video_channel_users if before.max_video_channel_users else keinerLocale, after=after.max_video_channel_users if after.max_video_channel_users else keinerLocale))

        if before.name != after.name:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.name", before=before.name, after=after.name))
        ...
        if before.nsfw_level != after.nsfw_level:
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.nsfwLevel", before=defaultLocale if before.nsfw_level.default else explicitLocale if before.nsfw_level.explicit else safeLocale if before.nsfw_level.safe else ageRegisteredLocale if before.nsfw_level.age_restricted else keinerLocale, after=defaultLocale if after.nsfw_level.default else explicitLocale if after.nsfw_level.explicit else safeLocale if after.nsfw_level.safe else ageRegisteredLocale if after.nsfw_level.age_restricted else keinerLocale))

        if before.owner != after.owner:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.owner", before=before.owner.mention if before.owner else keinerLocale, after=after.owner.mention if after.owner else keinerLocale))

        if before.preferred_locale != after.preferred_locale:
            beforeLocale = tanjunLocalizer.localize(locale, "logs.guildUpdate.preferredLocaleLocales." + str(before.preferred_locale))
            afterLocale = tanjunLocalizer.localize(locale, "logs.guildUpdate.preferredLocaleLocales." + str(after.preferred_locale))
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.preferredLocale", before=beforeLocale, after=afterLocale))    

        if before.premium_progress_bar_enabled != after.premium_progress_bar_enabled:
            if before.premium_progress_bar_enabled:
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.premiumProgressBarEnabled.activated"))
            else:
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.premiumProgressBarEnabled.deactivated"))

        if before.premium_subscriber_role != after.premium_subscriber_role:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.premiumSubscriberRole", before=before.premium_subscriber_role.mention if before.premium_subscriber_role else keinerLocale, after=after.premium_subscriber_role.mention if after.premium_subscriber_role else keinerLocale))    

        if before.premium_subscribers != after.premium_subscribers:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.premiumSubscribers", before=before.premium_subscribers, after=after.premium_subscribers))

        if before.premium_tier != after.premium_tier:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.premiumTier", before=before.premium_tier, after=after.premium_tier))

        if before.public_updates_channel != after.public_updates_channel:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.publicUpdatesChannel", before=before.public_updates_channel.mention if before.public_updates_channel else keinerLocale, after=after.public_updates_channel.mention if after.public_updates_channel else keinerLocale))

        if before.rules_channel != after.rules_channel:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.rulesChannel", before=before.rules_channel.mention if before.rules_channel else keinerLocale, after=after.rules_channel.mention if after.rules_channel else keinerLocale))

        if before.safety_alerts_channel != after.safety_alerts_channel:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.safetyAlertsChannel", before=before.safety_alerts_channel.mention if before.safety_alerts_channel else keinerLocale, after=after.safety_alerts_channel.mention if after.safety_alerts_channel else keinerLocale))
        ...
        if before.verification_level != after.verification_level:
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildUpdate.verificationLevel", before=noneLocale if before.verification_level.none else lowLocale if before.verification_level.low else mediumLocale if before.verification_level.medium else highLocale if before.verification_level.high else highestLocale, after=noneLocale if after.verification_level.none else lowLocale if after.verification_level.low else mediumLocale if after.verification_level.medium else highLocale if after.verification_level.high else highestLocale))
    ...
    async def on_invite_create(self, invite: discord.Invite):
        ...
        description_parts.append(tanjunLocalizer.localize(locale, "logs.inviteCreate.createdBy", created_by=invite.inviter.mention))
        description_parts.append(tanjunLocalizer.localize(locale, "logs.inviteCreate.expires", expires=neverLocale if invite.expires_at is None else "<t:" + str(utility.date_time_to_timestamp(invite.expires_at)) + ":R>"))
        description_parts.append(tanjunLocalizer.localize(locale, "logs.inviteCreate.max_uses", max_uses=infiniteLocale if invite.max_uses is None else invite.max_uses))

        if invite.channel:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.inviteCreate.channel", channel=invite.channel.mention))
        if invite.scheduled_event:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.inviteCreate.scheduledEvent", scheduled_event=invite.scheduled_event.url))
        if invite.target_application:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.inviteCreate.targetApplication", target_application=invite.target_application.name))

        if str(invite.target_type) != "InviteTarget.unknown":
            description_parts.append(tanjunLocalizer.localize(locale, "logs.inviteCreate.targetTypeLocales." + str(invite.target_type)))

        if invite.target_user:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.inviteCreate.targetUser", target_user=invite.target_user.mention))
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        description_parts.append(tanjunLocalizer.localize(locale, "logs.memberRemove.roles", roles=", ".join(role.mention for role in member.roles)))
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        if before.display_avatar != after.display_avatar:
            ...
            avatar_bytes = await before.display_avatar.read() if before.display_avatar else None  # Read the old avatar as bytes
            ...
            new_avatar_bytes = await after.display_avatar.read() if after.display_avatar else None  # Read the new avatar as bytes
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.userUpdate.avatar", 
        ...
        if before.banner != after.banner:
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.userUpdate.banner", 
                before=f"[{urlLocale}]({banner_url_before})", 
                after=f"[{urlLocale}]({new_banner_url})"))

        # Check for display name change
        if before.display_name != after.display_name:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.memberUpdate.displayName", before=before.display_name, after=after.display_name))
        ...
        if added_roles:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.memberUpdate.addedRoles", roles=", ".join(added_roles)))
        
        if removed_roles:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.memberUpdate.removedRoles", roles=", ".join(removed_roles)))
        ...
        if before.timed_out_until != after.timed_out_until:
            if before.timed_out_until is None:
                description_parts.append(tanjunLocalizer.localize(locale, "logs.memberUpdate.timeout", timeout=utility.date_time_to_timestamp(after.timed_out_until)))
            else:
    ...
    async def on_user_update(self, before: discord.User, after: discord.User):
        for guild in self.bot.guilds:
            ...
            if before.avatar != after.avatar:
                ...
                avatar_upload_response = await utility.upload_image_to_imgbb(avatar_bytes, "png") if avatar_bytes else {}
                avatar_url_before = avatar_upload_response.get("data", {}).get("url", defaultAvatarUrl)
                avatar_url_after = after.display_avatar.url if after.display_avatar else defaultAvatarUrl  # New avatar URL
            ...
            if before.banner != after.banner:
                ...
                banner_upload_response = await utility.upload_image_to_imgbb(banner_bytes, "png") if banner_bytes else {}
                banner_url_before = banner_upload_response.get("data", {}).get("url", noneLocale)
                banner_url_after = after.banner.url if after.banner else noneLocale  # New banner URL
    ...
    async def on_member_unban(self, guild: discord.Guild, user: discord.User):
        ...
        if unbanned_by:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.memberUnban.unbanned_by", unbanned_by=unbanned_by.mention))
    ...
    async def on_presence_update(self, before: discord.Member, after: discord.Member):
        ...
        if before.activity != after.activity:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.presenceUpdate.activity", before=before.activity, after=after.activity))
    ...
    async def on_message_edit(self, before: discord.Message, after: discord.Message):
        ...
        if before.content != after.content:
            ...
            truncated_notice = tanjunLocalizer.localize(locale, "logs.messageEdit.truncatedNotice")  # e.g., "... (truncated)"
        ...
        if before.attachments != after.attachments:
            edited_attachments = [f"[{attachment.filename}]({attachment.url})" for attachment in after.attachments if attachment not in before.attachments]
            if edited_attachments:
                description_parts.append(tanjunLocalizer.localize(locale, "logs.messageEdit.attachments", attachments=", ".join(edited_attachments)))
    ...
    async def on_message_delete(self, message: discord.Message):
        ...
        description_parts.append(tanjunLocalizer.localize(locale, "logs.messageDelete.name", user=message.author.mention, channel=message.channel.mention))
        deleted_by = None
        ...
        async for log in message.guild.audit_logs(limit=1, user=message.author):
            deleted_by = log.user
        if deleted_by:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.messageDelete.deletedBy", deleted_by=deleted_by.mention))
        ...
        if message.attachments:
            ...
            description_parts.append(tanjunLocalizer.localize(locale, "logs.messageDelete.attachments", attachments=attachments))
    ...
    async def on_reaction_add(self, reaction: discord.Reaction, user: discord.User):
        ...
        description_parts.append(tanjunLocalizer.localize(locale, "logs.reactionAdd.name", user=user.mention, emoji=reaction.emoji, message=reaction.message.jump_url))
    ...
    async def on_reaction_remove(self, reaction: discord.Reaction, user: discord.User):
        ...
        description_parts.append(tanjunLocalizer.localize(locale, "logs.reactionRemove.name", user=user.mention, emoji=reaction.emoji, message=reaction.message.jump_url))
    ...
    async def on_guild_role_create(self, role: discord.Role):
        ...
        async for log in role.guild.audit_logs(limit=1, user=role.guild.owner):
            created_by = log.user
        if created_by:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleCreate.createdBy", created_by=created_by.mention))
        ...
        if role.display_icon:
            if isinstance(role.display_icon, discord.Asset):
                urlLocale = tanjunLocalizer.localize(locale, "logs.userUpdate.guildAvatarLocales.url")
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleCreate.displayIcon", displayIcon=f"[{urlLocale}]({role.display_icon.url})"))
            else:
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleCreate.displayIcon", displayIcon=role.display_icon))
        ...
        if role.permissions:
            ...
            if permissions_list: 
                formatted_permissions = "\n".join([f"- `{tanjunLocalizer.localize(locale, f'logs.permissions.{perm}')}`" for perm in permissions_list])
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleCreate.permissions", permissions=formatted_permissions))
    ...
    async def on_guild_role_delete(self, role: discord.Role):
        ...
        async for log in role.guild.audit_logs(limit=1, user=role.guild.owner):
            deleted_by = log.user
        if deleted_by:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleDelete.deletedBy", deleted_by=deleted_by.mention))
        ...
        if role.permissions:
            ...
            if permissions_list: 
                formatted_permissions = "\n".join([f"- `{tanjunLocalizer.localize(locale, f'logs.permissions.{perm}')}`" for perm in permissions_list])
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleDelete.permissions", permissions=formatted_permissions))
    ...
    async def on_guild_role_update(self, before: discord.Role, after: discord.Role):
        ...
        async for log in after.guild.audit_logs(limit=1, user=after.guild.owner):
            updated_by = log.user
        if updated_by:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleUpdate.updatedBy", updated_by=updated_by.mention))

        if before.color != after.color:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleUpdate.color", before=before.color, after=after.color))

        if before.hoist != after.hoist:
            if after.hoist:
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleUpdate.hoistNow", role=after.name))
            else:
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleUpdate.hoistNoLonger", role=after.name))

        if before.mentionable != after.mentionable:
            if after.mentionable:
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleUpdate.mentionableNow", role=after.name))
            else:
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleUpdate.mentionableNoLonger", role=after.name))
        ...
        if before.managed != after.managed:
            if after.managed:
                ...
                description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleUpdate.managedNoLonger", role=after.name))
        ...
        if added_perms:
            added_perms_list = ", ".join([tanjunLocalizer.localize(locale, f'logs.permissions.{perm}') for perm in added_perms])
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleUpdate.addedPermissions", permissions=added_perms_list))

        if removed_perms:
            removed_perms_list = ", ".join([tanjunLocalizer.localize(locale, f'logs.permissions.{perm}') for perm in removed_perms])
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleUpdate.removedPermissions", permissions=removed_perms_list))

        if before.display_icon != after.display_icon:
            urlLocale = tanjunLocalizer.localize(locale, "logs.userUpdate.guildAvatarLocales.url")
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleUpdate.displayIcon", before=f"[{urlLocale}]({before.display_icon.url})" if before.display_icon else "None", after=f"[{urlLocale}]({after.display_icon.url})" if after.display_icon else "None"))

        if before.icon != after.icon:
            description_parts.append(tanjunLocalizer.localize(locale, "logs.guildRoleUpdate.icon", before=before.icon, after=after.icon))
    ...
    async def on_ready(self):
        logcmds = LogsCommands(name="logs", description="Log Commands")
