...
class TwitchService:
    ...
    async def get_notifications(self, channel_id: str) -> list[TwitchNotification]:
        ...
        query = (
            ...
            "FROM twitchOnlineNotification WHERE channel_id = %s"
        )
        params = (channel_id,)
    ...
    async def add_notification(
        ...
        query = (
            "INSERT INTO twitchOnlineNotification "
            "(guild_id, channel_id, twitchUuid, twitchName, notification_message) "
            "VALUES (%s, %s, %s, %s, %s)"
        )
    ...
    async def get_notifications_by_guild(self, guild_id: str) -> list[TwitchNotification]:
        ...
        query = (
            "SELECT id, channel_id, guild_id, twitchUuid, twitchName, notification_message "
            "FROM twitchOnlineNotification WHERE guild_id = %s"
        )
        params = (guild_id,)
        rows: list[TwitchNotification] = []
        async for row in execute_query_iter(query, params):
