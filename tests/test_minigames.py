...
class TestModeMap:
    ...
    def test_mode_map_keys_are_consecutive(self):
        assert list(modeMap.keys()) == list(range(1, 15))


class TestPrimes:
    def test_primes_list_starts_correctly(self):
        assert primes[1] == 2
        assert primes[2] == 3
        assert primes[3] == 5
        assert primes[4] == 7

    def test_primes_list_values_are_prime(self):
        for p in primes[1:]:
            assert all(p % d != 0 for d in range(2, int(math.sqrt(p)) + 1))

    def test_primes_list_has_zero_sentinel(self):
        assert primes[0] == 0

    def test_primes_includes_common_values(self):
        assert 2 in primes
        assert 3 in primes
        assert 5 in primes
        assert 7 in primes
...
class TestFibonacci:
    def test_fibonacci_starting_values(self):
    ...
    def test_fibonacci_sentinel_value(self):
        """Index 0 has sentinel -1."""
        assert fibonacci[0] == -1

    def test_fibonacci_values_increase(self):
...
class TestRomealToNumber:
    ...
    def test_subtractive_pair_in_middle(self):
...
class TestGetGoal:
    def test_all_modes_return_number(self):
        for mode in range(1, 15):
            result = get_goal(mode)
            assert result is not None

    def test_normal_goal_range(self):
        for _ in range(50):
            goal = get_goal(1)
            assert 20 <= goal <= 100

    def test_negative_goal_range(self):
        for _ in range(50):
            goal = get_goal(2)
            assert -100 <= goal <= -20

    def test_reverse_goal_range(self):
        for _ in range(50):
            goal = get_goal(3)
            assert 5 <= goal <= 80

    def test_prime_goal_is_prime(self):
        for _ in range(20):
            goal = get_goal(4)
            assert goal in primes

    def test_even_goal_is_even(self):
        for _ in range(20):
            goal = get_goal(5)
            assert goal % 2 == 0

    def test_odd_goal_is_odd(self):
        for _ in range(20):
            goal = get_goal(6)
            assert goal % 2 != 0

    def test_fibonacci_goal_in_sequence(self):
        for _ in range(20):
            goal = get_goal(7)
            assert goal in fibonacci

    def test_double_goal_is_power_of_two(self):
        for _ in range(20):
            goal = get_goal(8)
            assert goal & (goal - 1) == 0

    def test_triple_goal_is_power_of_three(self):
        for _ in range(20):
            goal = get_goal(9)
            n = goal
            while n % 3 == 0:
    ...
    def test_hundreds_goal_divisible_by_100(self):
        for _ in range(20):
            goal = get_goal(10)
            assert goal % 100 == 0

    def test_square_goal_is_perfect_square(self):
        for _ in range(20):
            goal = get_goal(13)
            assert math.isqrt(goal) ** 2 == goal

    def test_cube_goal_is_perfect_cube(self):
        for _ in range(20):
            goal = get_goal(14)
...
class TestRomealToNumberDeepEdgeCases:
    def test_invalid_four_repeats_various(self):
    ...
    def test_subtractive_pairs_all(self):
        assert romeal_to_number("IV") == 4
        assert romeal_to_number("IX") == 9
        assert romeal_to_number("XL") == 40
        assert romeal_to_number("XC") == 90
    ...
    def test_single_numeral_each(self):
        assert romeal_to_number("I") == 1
        assert romeal_to_number("V") == 5
        assert romeal_to_number("X") == 10
        assert romeal_to_number("L") == 50
...
class TestNumberToRomealDeepEdgeCases:
    def test_all_basic_numerals(self):
        assert number_to_romeal(1) == "I"
        assert number_to_romeal(5) == "V"
        assert number_to_romeal(10) == "X"
        assert number_to_romeal(50) == "L"
    ...
    def test_round_trip_all_primes_under_100(self):
...
class TestGetCorrectNextNumberDeepEdgeCases:
    def test_normal_mode_starts_at_zero(self):
        assert get_correct_next_number(1, 0) == 1

    def test_negative_mode_starts_at_zero(self):
        assert get_correct_next_number(2, 0) == -1

    def test_reverse_mode_starts_at_101(self):
        assert get_correct_next_number(3, 101) == 100

    def test_prime_mode_starts_at_sentinel(self):
        """Mode 4 starts with sentinel 0, next is 2 (first prime)."""
        assert get_correct_next_number(4, 0) == 2

    def test_even_mode_starts_at_zero(self):
        assert get_correct_next_number(5, 0) == 2

    def test_odd_mode_starts_at_negative_one(self):
        assert get_correct_next_number(6, -1) == 1

    def test_fibonacci_mode_sentinel(self):
        """Mode 7 has a -1 sentinel and a -15 sentinel for the second 1."""
        assert get_correct_next_number(7, -1) == 0
        assert get_correct_next_number(7, -15) == 1

    def test_double_mode_starts_at_one(self):
        assert get_correct_next_number(8, 1) == 2

    def test_triple_mode_starts_at_one(self):
        assert get_correct_next_number(9, 1) == 3

    def test_hundreds_mode_starts_at_zero(self):
        assert get_correct_next_number(10, 0) == 100

    def test_square_mode_starts_at_zero(self):
        assert get_correct_next_number(13, 0) == 1

    def test_cube_mode_starts_at_zero(self):
        assert get_correct_next_number(14, 0) == 1

    def test_binary_mode_sequence(self):
    ...
    def test_square_mode_sequence(self):
        assert get_correct_next_number(13, 0) == 1
        assert get_correct_next_number(13, 1) == 4
        assert get_correct_next_number(13, 4) == 9
        assert get_correct_next_number(13, 9) == 16

    def test_cube_mode_sequence(self):
        assert get_correct_next_number(14, 0) == 1
        assert get_correct_next_number(14, 1) == 8
        assert get_correct_next_number(14, 8) == 27
