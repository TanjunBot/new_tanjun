...
class TestEmbedProxy:
    ...
    def test_repr_hides_private(self) -> None:
    ...
    def test_equality_with_none(self) -> None:
        proxy = EmbedProxy({"a": 1})
        assert proxy != None  # noqa: E711

    def test_len_with_nested_values(self) -> None:
...
class TestTanjunEmbedConstruction:
    ...
    def test_default_colour_is_set(self) -> None:
        embed = tanjunEmbed()
        assert embed.colour is not None
        assert embed.colour.value == 0xCB33F5

    def test_default_colour_makes_embed_truthy(self) -> None:
        embed = tanjunEmbed()
    ...
    def test_colour_set_to_none(self) -> None:
        embed = tanjunEmbed(colour=0xFF0000)
        embed.colour = None
        assert embed.colour is None

    def test_colour_discord_colour_object(self) -> None:
        import discord
    ...
    def test_colour_invalid_type_raises(self) -> None:
        embed = tanjunEmbed()
        with pytest.raises(TypeError):
            embed.colour = "invalid"

    def test_str_conversion_in_constructor(self) -> None:
...
class TestTanjunEmbedFooter:
    def test_set_footer_text_and_icon(self) -> None:
        embed = tanjunEmbed()
    ...
    def test_remove_footer_when_not_set(self) -> None:
        embed = tanjunEmbed()
        # Should not raise
        embed.remove_footer()
        assert embed.footer.text is None
...
class TestTanjunEmbedMedia:
    ...
    def test_set_image_none_removes(self) -> None:
        embed = tanjunEmbed()
        embed.set_image(url="http://example.com/image.png")
    ...
    def test_set_thumbnail_none_removes(self) -> None:
        embed = tanjunEmbed()
        embed.set_thumbnail(url="http://example.com/thumb.png")
        embed.set_thumbnail(url=None)
        assert embed.thumbnail.url is None

    def test_video_property_returns_none_for_missing(self) -> None:
        embed = tanjunEmbed()
        assert embed.video.url is None

    def test_provider_property_returns_none_for_missing(self) -> None:
        embed = tanjunEmbed()
        assert embed.provider.name is None


class TestTanjunEmbedAuthor:
    def test_set_author_all_params(self) -> None:
        embed = tanjunEmbed()
    ...
    def test_set_author_name_only(self) -> None:
        embed = tanjunEmbed()
    ...
    def test_remove_author_when_not_set(self) -> None:
        embed = tanjunEmbed()
        embed.remove_author()
        assert embed.author.name is None
...
class TestTanjunEmbedFields:
    ...
    def test_add_field_default_inline(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="F1", value="V1")
        assert embed.fields[0].inline is True

    def test_add_field_inline_false(self) -> None:
        embed = tanjunEmbed()
    ...
    def test_insert_field_at(self) -> None:
        embed = tanjunEmbed()
    ...
    def test_clear_fields_when_empty(self) -> None:
        embed = tanjunEmbed()
        embed.clear_fields()
        assert len(embed.fields) == 0
    ...
    def test_remove_field_invalid_index_silent(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="F1", value="V1")
        embed.remove_field(99)  # Should silently pass
        assert len(embed.fields) == 1

    def test_remove_field_from_empty(self) -> None:
        embed = tanjunEmbed()
        embed.remove_field(0)  # Should silently pass
        assert len(embed.fields) == 0

    def test_set_field_at(self) -> None:
        embed = tanjunEmbed()
    ...
    def test_set_field_at_invalid_index_raises(self) -> None:
        embed = tanjunEmbed()
...
class TestTanjunEmbedLenAndBool:
    def test_len_title_and_description(self) -> None:
        embed = tanjunEmbed(title="Hello", description="World")
        assert len(embed) == 10  # 5 + 5

    def test_len_with_fields(self) -> None:
        embed = tanjunEmbed(title="Hi")
        embed.add_field(name="F1", value="V1")
        assert len(embed) == 6  # 2 (title) + 2 (name) + 2 (value)

    def test_len_with_footer(self) -> None:
        embed = tanjunEmbed()
        embed.set_footer(text="A" * 50)
        assert len(embed) == 50

    def test_len_with_author(self) -> None:
        embed = tanjunEmbed()
        embed.set_author(name="AuthorName")
        assert len(embed) == 10

    def test_len_empty_embed(self) -> None:
        embed = tanjunEmbed()
        # Default colour doesn't count toward length
        assert len(embed) == 0

    def test_bool_default_colour_is_truthy(self) -> None:
        embed = tanjunEmbed()
        assert bool(embed) is True

    def test_bool_with_title_is_truthy(self) -> None:
        embed = tanjunEmbed(colour=None, title="Test")
        assert bool(embed) is True

    def test_bool_with_url_is_truthy(self) -> None:
        embed = tanjunEmbed(colour=None, url="http://example.com")
        assert bool(embed) is True

    def test_bool_with_description_is_truthy(self) -> None:
        embed = tanjunEmbed(colour=None, description="Desc")
        assert bool(embed) is True

    def test_bool_with_field_is_truthy(self) -> None:
        embed = tanjunEmbed(colour=None)
        embed.add_field(name="F", value="V")
        assert bool(embed) is True

    def test_bool_with_timestamp_is_truthy(self) -> None:
...
class TestTanjunEmbedEquality:
    ...
    def test_equality_not_implemented_for_different_type(self) -> None:
        embed = tanjunEmbed(title="Test")
...
class TestTanjunEmbedFromDict:
    ...
    def test_from_dict_with_footer(self) -> None:
...
class TestTanjunEmbedToDict:
    ...
    def test_to_dict_with_none_colour(self) -> None:
        embed = tanjunEmbed()
        embed.colour = None
        d = embed.to_dict()
...
class TestTanjunEmbedCopy:
    def test_copy_preserves_title(self) -> None:
        embed = tanjunEmbed(title="Original")
        copy = embed.copy()
        assert copy.title == "Original"

    def test_copy_preserves_fields_count(self) -> None:
        embed = tanjunEmbed(colour=None, title="Original")
        embed.add_field(name="F1", value="V1")
        copy = embed.copy()
        assert len(copy.fields) == 1

    def test_copy_is_independent_colour(self) -> None:
        embed = tanjunEmbed(colour=0xFF0000)
        copy = embed.copy()
        copy.colour = None
        assert embed.colour is not None
...
class TestGetXpForLevel:
    ...
    def test_custom_formula_returns_non_negative(self) -> None:
        xp = get_xp_for_level(5, "custom", custom_formula="100*5")
        assert xp >= 0

    def test_custom_formula_invalid(self) -> None:
        assert get_xp_for_level(5, "custom", custom_formula="invalid!!") == 0

    def test_default_scaling_is_medium(self) -> None:
        xp = get_xp_for_level(5, "nonexistent")
        expected = math.floor(100 * (5**1.5))
        assert xp == expected

    def test_consistency_with_level_for_xp(self) -> None:
...
class TestGetLevelForXp:
    ...
    def test_small_xp_easy(self) -> None:
...
class TestNumericStringParser:
    ...
    def test_function_cos(self) -> None:
        parser = NumericStringParser()
        assert abs(parser.eval("cos(0)") - 1.0) < 0.01

    def test_function_tan(self) -> None:
        parser = NumericStringParser()
    ...
    def test_function_asin(self) -> None:
        parser = NumericStringParser()
        assert abs(parser.eval("asin(0)")) < 0.01

    def test_function_acos(self) -> None:
        parser = NumericStringParser()
        assert abs(parser.eval("acos(1)")) < 0.01

    def test_function_atan(self) -> None:
        parser = NumericStringParser()
    ...
    def test_function_log10(self) -> None:
        parser = NumericStringParser()
        assert abs(parser.eval("log10(100)") - 2.0) < 0.01

    def test_function_log2(self) -> None:
        parser = NumericStringParser()
        assert abs(parser.eval("log2(8)") - 3.0) < 0.01

    def test_function_exp(self) -> None:
        parser = NumericStringParser()
...
class TestRelativeTimeStrToDate:
    ...
    def test_only_numbers_no_units(self) -> None:
...
class TestRelativeTimeToSeconds:
    ...
    def test_repeated_units_adds(self) -> None:
...
class TestCmp:
    ...
    def test_equal_negative(self) -> None:
        assert cmp(-5, -5) == 0
...
class TestCheckIfStrIsHexColor:
    ...
    def test_valid_hex_short(self) -> None:
    ...
    def test_hex_with_valid_prefix(self) -> None:
        # "abcdef" is valid hex
...
class TestSqrtN:
    ...
    def test_default_n_is_2(self) -> None:
        assert abs(sqrt_n(9) - 3.0) < 0.01

    def test_fourth_root(self) -> None:
...
class TestCheckIfHasPro:
    ...
    def test_nonzero_returns_true(self) -> None:
        from utility import checkIfHasPro
...
class TestCommandInfo:
    def test_command_info_stores_attributes(self) -> None:
...
class TestCmpEdgeCases:
    def test_cmp_negative_numbers(self) -> None:
        assert cmp(-5, 3) == -1
        assert cmp(5, -3) == 1
        assert cmp(-5, -5) == 0

    def test_cmp_with_zero(self) -> None:
        assert cmp(0, 0) == 0
        assert cmp(0, 1) == -1
        assert cmp(1, 0) == 1

    def test_cmp_large_numbers(self) -> None:
        assert cmp(10**18, 10**18) == 0
        assert cmp(10**18 + 1, 10**18) == 1

    def test_cmp_floats(self) -> None:
...
class TestCheckIfStrIsHexColorEdgeCases:
    def test_valid_six_char_hex(self) -> None:
        assert check_if_str_is_hex_color("ff0000") is True

    def test_valid_three_char_hex(self) -> None:
        assert check_if_str_is_hex_color("f00") is True
...
class TestRelativeTimeStrToDateEdgeCases:
    ...
    def test_repeated_units(self) -> None:
...
class TestGetLevelForXpEdgeCases:
    def test_zero_xp_returns_level_1(self) -> None:
    ...
    def test_unknown_scaling_defaults_to_medium(self) -> None:
        xp = 100
    ...
    def test_consistency_with_xp_for_level(self) -> None:
...
class TestNumericStringParserDeepEdgeCases:
    ...
    def test_fn_dict_has_expected_functions(self) -> None:
        parser = NumericStringParser()
        expected = [
            "sin",
            "cos",
    ...
    def test_opn_dict_has_expected_operators(self) -> None:
        parser = NumericStringParser()
...
class TestEvalExprEdgeCases:
    ...
    def test_with_level_variable(self) -> None:
...
class TestEmbedProxyDeep:
    ...
    def test_getattr_returns_value_for_existing(self) -> None:
        proxy = EmbedProxy({"text": "hello"})
        assert proxy.text == "hello"

    def test_hasattr_always_true(self) -> None:
    ...
    def test_inequality_with_non_proxy(self) -> None:
        proxy = EmbedProxy({"text": "hello"})
        assert proxy != "not a proxy"


class TestTanjunEmbedDeep:
    def test_colour_setter_with_int(self) -> None:
        embed = tanjunEmbed()
        embed.colour = 0xFF0000
        assert embed.colour.value == 0xFF0000

    def test_colour_setter_with_none(self) -> None:
        embed = tanjunEmbed(colour=0xFF0000)
        embed.colour = None
        assert embed._colour is None

    def test_colour_setter_with_discord_colour(self) -> None:
        import discord
    ...
    def test_set_image_with_none(self) -> None:
        embed = tanjunEmbed()
        embed.set_image(url="http://example.com/img.png")
        embed.set_image(url=None)
        assert embed.image.url is None

    def test_set_thumbnail_with_none(self) -> None:
        embed = tanjunEmbed()
    ...
    def test_remove_field_index_error(self) -> None:
        """remove_field silently swallows IndexError."""
        embed = tanjunEmbed()
        embed.remove_field(99)  # Should not raise

    def test_set_field_at_index_error(self) -> None:
        embed = tanjunEmbed()
        with pytest.raises(IndexError):
            embed.set_field_at(0, name="x", value="y")

    def test_to_dict_includes_colour(self) -> None:
        embed = tanjunEmbed(colour=0xFF0000)
        d = embed.to_dict()
        assert "color" in d
        assert d["color"] == 0xFF0000

    def test_to_dict_black_colour_included(self) -> None:
    ...
    def test_bool_no_colour_no_content(self) -> None:
...
class TestNumericStringParserRoundAndFactorial:
    ...
    def test_factorial_direct(self) -> None:
        parser = NumericStringParser()
        assert parser.fn["factorial"](5) == 120
        assert parser.fn["factorial"](0) == 1
        assert parser.fn["factorial"](1) == 1

    def test_fac_alias_direct(self) -> None:
        """fac is an alias for factorial."""
        parser = NumericStringParser()
        assert parser.fn["fac"] is parser.fn["factorial"]
        assert parser.fn["fac"](5) == 120

    def test_sgn_direct_positive(self) -> None:
        parser = NumericStringParser()
        assert parser.fn["sgn"](5) == 1

    def test_sgn_direct_negative(self) -> None:
        parser = NumericStringParser()
        assert parser.fn["sgn"](-5) == -1

    def test_sgn_direct_zero(self) -> None:
        parser = NumericStringParser()
        assert parser.fn["sgn"](0) == 0

    def test_sgn_direct_small_positive(self) -> None:
        parser = NumericStringParser()
        assert parser.fn["sgn"](0.001) == 1

    def test_sgn_direct_small_negative(self) -> None:
        parser = NumericStringParser()
        assert parser.fn["sgn"](-0.001) == -1

    def test_sgn_direct_tiny_value(self) -> None:
...
class TestNumericStringParserFnDict:
    ...
    def test_pi_and_e_are_floats(self) -> None:
        parser = NumericStringParser()
...
class TestNumericStringParserEvalAdditional:
    ...
    def test_negative_number_in_expression(self) -> None:
        parser = NumericStringParser()
    ...
    def test_multiple_multiplications(self) -> None:
        parser = NumericStringParser()
    ...
    def test_double_parentheses(self) -> None:
        parser = NumericStringParser()
    ...
    def test_log10_with_power(self) -> None:
        parser = NumericStringParser()
        result = parser.eval("log10(100)")
        assert abs(result - 2.0) < 0.01

    def test_log2_with_power(self) -> None:
        parser = NumericStringParser()
    ...
    def test_trunc_positive(self) -> None:
        parser = NumericStringParser()
    ...
    def test_addition_with_decimal(self) -> None:
        parser = NumericStringParser()
...
class TestEvalExprSubstitutions:
    ...
    def test_pi_substitution(self) -> None:
        result = eval_expr("pi")
        assert abs(result - math.pi) < 0.001

    def test_e_substitution(self) -> None:
        result = eval_expr("e")
        assert abs(result - math.e) < 0.001

    def test_sqrt_substitution(self) -> None:
        result = eval_expr("sqrt(16)")
        assert abs(result - 4.0) < 0.001

    def test_sqrt_n_substitution(self) -> None:
        result = eval_expr("sqrt[3](27)")
        assert abs(result - 3.0) < 0.01

    def test_nthroot_substitution(self) -> None:
        result = eval_expr("nthroot[3](27)")
        assert abs(result - 3.0) < 0.01

    def test_log2_substitution(self) -> None:
        result = eval_expr("log2(8)")
        assert abs(result - 3.0) < 0.01

    def test_log10_substitution(self) -> None:
        result = eval_expr("log10(100)")
        assert abs(result - 2.0) < 0.01

    def test_ln_substitution(self) -> None:
        result = eval_expr("ln(2.718281828)")
        assert abs(result - 1.0) < 0.01

    def test_log_n_substitution(self) -> None:
        result = eval_expr("log[10](100)")
        assert abs(result - 2.0) < 0.01

    def test_sin_substitution(self) -> None:
        result = eval_expr("sin(0)")
        assert abs(result) < 0.001

    def test_cos_substitution(self) -> None:
        result = eval_expr("cos(0)")
        assert abs(result - 1.0) < 0.001

    def test_tan_substitution(self) -> None:
        result = eval_expr("tan(0)")
        assert abs(result) < 0.001

    def test_asin_substitution(self) -> None:
        result = eval_expr("asin(0)")
        assert abs(result) < 0.001

    def test_acos_substitution(self) -> None:
        result = eval_expr("acos(1)")
        assert abs(result) < 0.001

    def test_atan_substitution(self) -> None:
        result = eval_expr("atan(0)")
        assert abs(result) < 0.001

    def test_floor_substitution(self) -> None:
        result = eval_expr("floor(3.7)")
        assert result == 3

    def test_ceil_substitution(self) -> None:
        result = eval_expr("ceil(3.2)")
        assert result == 4

    def test_abs_substitution(self) -> None:
    ...
    def test_custom_formula_in_get_xp_for_level(self) -> None:
...
class TestTanjunEmbedLenDeep:
    def test_len_with_title_and_description(self) -> None:
        embed = tanjunEmbed(title="Hello", description="World")
        assert len(embed) == 10

    def test_len_with_fields_includes_names_and_values(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="Name", value="Value")
        total = len("Name") + len("Value")
        assert len(embed) == total

    def test_len_with_multiple_fields(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="A", value="1")
    ...
    def test_len_with_footer_only_text(self) -> None:
        embed = tanjunEmbed()
        embed.set_footer(text="FooterText")
        assert len(embed) == len("FooterText")

    def test_len_with_author_only_name(self) -> None:
        embed = tanjunEmbed()
        embed.set_author(name="AuthorName")
        assert len(embed) == len("AuthorName")

    def test_len_with_footer_and_author(self) -> None:
        embed = tanjunEmbed()
        embed.set_footer(text="Footer")
        embed.set_author(name="Author")
        assert len(embed) == len("Footer") + len("Author")

    def test_len_with_all_components(self) -> None:
...
class TestTanjunEmbedBoolDeep:
    def test_bool_with_author_is_truthy(self) -> None:
        embed = tanjunEmbed(colour=None, color=None)
        embed.set_author(name="Author")
        assert bool(embed) is True

    def test_bool_with_thumbnail_is_truthy(self) -> None:
        embed = tanjunEmbed(colour=None, color=None)
        embed.set_thumbnail(url="http://example.com/thumb.png")
        assert bool(embed) is True

    def test_bool_with_footer_is_truthy(self) -> None:
        embed = tanjunEmbed(colour=None, color=None)
        embed.set_footer(text="Footer")
        assert bool(embed) is True

    def test_bool_with_image_is_truthy(self) -> None:
        embed = tanjunEmbed(colour=None, color=None)
        embed.set_image(url="http://example.com/img.png")
        assert bool(embed) is True

    def test_bool_with_video_proxy_is_falsy(self) -> None:
    ...
    def test_bool_with_provider_proxy(self) -> None:
        embed = tanjunEmbed(colour=None, color=None)
        assert bool(embed) is False

    def test_bool_after_removing_all_content(self) -> None:
...
class TestTanjunEmbedFromDictDeep:
    def test_from_dict_with_video(self) -> None:
    ...
    def test_from_dict_with_url(self) -> None:
...
class TestTanjunEmbedToDictDeep:
    ...
    def test_to_dict_empty_title_not_included(self) -> None:
        embed = tanjunEmbed(colour=None, color=None)
        d = embed.to_dict()
        assert "title" not in d

    def test_to_dict_empty_description_not_included(self) -> None:
        embed = tanjunEmbed(colour=None, color=None)
        d = embed.to_dict()
    ...
    def test_to_dict_with_author(self) -> None:
        embed = tanjunEmbed()
        embed.set_author(name="Author", url="http://link.com")
        d = embed.to_dict()
        assert "author" in d

    def test_to_dict_with_image(self) -> None:
        embed = tanjunEmbed()
        embed.set_image(url="http://img.png")
        d = embed.to_dict()
        assert "image" in d

    def test_to_dict_with_thumbnail(self) -> None:
        embed = tanjunEmbed()
...
class TestTanjunEmbedCopyDeep:
    def test_copy_preserves_description(self) -> None:
        embed = tanjunEmbed(description="My description")
        copy = embed.copy()
        assert copy.description == "My description"

    def test_copy_preserves_url(self) -> None:
        embed = tanjunEmbed(url="http://example.com")
        copy = embed.copy()
    ...
    def test_copy_preserves_footer(self) -> None:
        embed = tanjunEmbed()
        embed.set_footer(text="Footer text")
        copy = embed.copy()
        assert copy.footer.text == "Footer text"

    def test_copy_preserves_author(self) -> None:
        embed = tanjunEmbed()
    ...
    def test_copy_preserves_field_count(self) -> None:
    ...
    def test_copy_preserves_image(self) -> None:
        embed = tanjunEmbed()
        embed.set_image(url="http://img.png")
        copy = embed.copy()
        assert copy.image.url == "http://img.png"

    def test_copy_preserves_thumbnail(self) -> None:
        embed = tanjunEmbed()
        embed.set_thumbnail(url="http://thumb.png")
        copy = embed.copy()
...
class TestTanjunEmbedSetAuthorOverwrite:
    def test_set_author_overwrites_previous(self) -> None:
        embed = tanjunEmbed()
        embed.set_author(name="First")
        embed.set_author(name="Second")
        assert embed.author.name == "Second"

    def test_set_author_with_url(self) -> None:
        embed = tanjunEmbed()
        embed.set_author(name="Author", url="http://link.com")
        assert embed.author.url == "http://link.com"

    def test_set_author_returns_self(self) -> None:
        embed = tanjunEmbed()
        result = embed.set_author(name="Author")
        assert result is embed

    def test_set_author_icon_url(self) -> None:
        embed = tanjunEmbed()
...
class TestTanjunEmbedAddFieldChaining:
    ...
    def test_add_field_chaining(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="F1", value="V1").add_field(name="F2", value="V2").add_field(name="F3", value="V3")
        assert len(embed.fields) == 3

    def test_insert_field_at_returns_self(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="F1", value="V1")
        result = embed.insert_field_at(0, name="F0", value="V0")
        assert result is embed

    def test_clear_fields_returns_self(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="F1", value="V1")
        result = embed.clear_fields()
        assert result is embed

    def test_remove_field_returns_self(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="F1", value="V1")
        result = embed.remove_field(0)
        assert result is embed

    def test_set_field_at_returns_self(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="F1", value="V1")
...
class TestTanjunEmbedFieldValues:
    def test_field_values_are_stringified(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name=123, value=456)
        assert embed.fields[0].name == "123"
        assert embed.fields[0].value == "456"

    def test_field_inline_default_true(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="F", value="V")
        assert embed.fields[0].inline is True

    def test_field_inline_false(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="F", value="V", inline=False)
        assert embed.fields[0].inline is False

    def test_set_field_at_changes_all_attributes(self) -> None:
        embed = tanjunEmbed()
    ...
    def test_insert_field_at_middle(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="First", value="1")
    ...
    def test_remove_field_middle(self) -> None:
        embed = tanjunEmbed()
        embed.add_field(name="A", value="1")
        embed.add_field(name="B", value="2")
...
class TestDrawTextWithOutline:
    def test_draw_text_with_outline_calls_draw_six_times(self) -> None:
    ...
    def test_outline_uses_outline_color(self) -> None:
        from utility import draw_text_with_outline
    ...
    def test_text_uses_text_color(self) -> None:
        from utility import draw_text_with_outline
    ...
    def test_outline_positions(self) -> None:
        from utility import draw_text_with_outline
