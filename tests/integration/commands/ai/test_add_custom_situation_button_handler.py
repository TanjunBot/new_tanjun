...
async def test_situation_not_found(handler, custom_id: str) -> None:
    ...
    with patch(
        "commands.ai.add_custom_situation_button_handler.AiService.get_user_situation", new=AsyncMock(return_value=None)
    ):
...
async def test_creator_gone(handler, custom_id: str, use_client: bool) -> None:
    ...
    with patch(
        "commands.ai.add_custom_situation_button_handler.AiService.get_user_situation", new=AsyncMock(return_value={"id": "1"})
    ):
        await handler(interaction)
...
async def test_approve_success() -> None:
    ...
    with (
        patch(
            "commands.ai.add_custom_situation_button_handler.AiService.get_user_situation",
