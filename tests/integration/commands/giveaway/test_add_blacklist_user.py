"""Integration tests for commands.giveaway.add_blacklist_user."""
...
from commands.giveaway.add_blacklist_user import add_blacklist_user as command_fn
...
@pytest.mark.asyncio
@patch("commands.giveaway.add_blacklist_user.giveaway_service")
async def test_success_add(mock_svc):
    ...
    await command_fn(info, make_user())
    embed_from_reply(info.reply)
    mock_svc.add_blacklisted_user.assert_awaited_once()


@pytest.mark.asyncio
@patch("commands.giveaway.add_blacklist_user.giveaway_service")
async def test_already_exists(mock_svc):
    entity = MagicMock(entity_id="333333333")
