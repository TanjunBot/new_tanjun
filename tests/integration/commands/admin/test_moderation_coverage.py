...
from tests.helpers.discord import make_permissions, make_role, make_target_member, make_text_channel
...
async def test_nuke_wrong_confirmation(admin_command_info):
    ...
    async def capture(*args, **kwargs):
        ...
        return MagicMock()
    admin_command_info.reply = AsyncMock(side_effect=capture)
    admin_command_info.client.wait_for = AsyncMock(return_value=MagicMock(content='wrongword', author=admin_command_info.user, channel=admin_command_info.channel))
    await nuke_channel(admin_command_info, channel=channel)
    admin_command_info.channel.send.assert_awaited()
...
async def test_nuke_clone_forbidden(admin_command_info):
    ...
    async def capture(*args, **kwargs):
        ...
        return MagicMock()
    admin_command_info.reply = AsyncMock(side_effect=capture)
    word = locale.commands.admin.nuke.confirmationWord(str(admin_command_info.locale))
    admin_command_info.client.wait_for = AsyncMock(return_value=MagicMock(content=word, author=admin_command_info.user, channel=admin_command_info.channel))
    await nuke_channel(admin_command_info, channel=channel)
    admin_command_info.channel.send.assert_awaited()
...
@pytest.mark.parametrize('setting', ['all', 'bot', 'user', 'notPinned', 'userNotPinned', 'botNotPinned', 'notAdmin', 'userNotAdmin', 'embeds', 'files', 'notAdminNotPinned'])
