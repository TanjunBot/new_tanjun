...
async def test_root_command_name_is_minigame_name():
    bot = await load_extension_bot(EXTENSION)
    assert get_tree_command_names(bot) == ["minigame_name"]
...
async def test_subcommand_minigames_countingcmds_name_registered():
    bot = await load_extension_bot(EXTENSION)
    root = find_tree_group(bot, "minigame_name")
    assert root is not None
    assert "minigames_countingcmds_name" in get_subcommand_names(root)

async def test_subcommand_minigames_cchcmds_name_registered():
    bot = await load_extension_bot(EXTENSION)
    root = find_tree_group(bot, "minigame_name")
    assert root is not None
    assert "minigames_cchcmds_name" in get_subcommand_names(root)

async def test_subcommand_minigames_cmodescmds_name_registered():
    bot = await load_extension_bot(EXTENSION)
    root = find_tree_group(bot, "minigame_name")
    assert root is not None
    assert "minigames_cmodescmds_name" in get_subcommand_names(root)

async def test_subcommand_minigames_wordchaincmds_name_registered():
    bot = await load_extension_bot(EXTENSION)
    root = find_tree_group(bot, "minigame_name")
    assert root is not None
    assert "minigames_wordchaincmds_name" in get_subcommand_names(root)
