...
async def test_root_command_name_is_logs_name():
...
async def test_subcommand_logs_set_name_registered():
    ...
    root = find_tree_group(bot, "logs_name")
    assert root is not None
    assert "logs_set_name" in get_subcommand_names(root)


async def test_subcommand_logs_remove_name_registered():
    bot = await load_extension_bot(EXTENSION)
    root = find_tree_group(bot, "logs_name")
    assert root is not None
    assert "logs_remove_name" in get_subcommand_names(root)


async def test_subcommand_logs_configure_name_registered():
    bot = await load_extension_bot(EXTENSION)
    root = find_tree_group(bot, "logs_name")
    assert root is not None
    assert "logs_configure_name" in get_subcommand_names(root)


async def test_subcommand_logs_blacklist_name_registered():
    bot = await load_extension_bot(EXTENSION)
    root = find_tree_group(bot, "logs_name")
    assert root is not None
    assert "logs_blacklist_name" in get_subcommand_names(root)


async def test_subcommand_logs_blacklistu_name_registered():
    bot = await load_extension_bot(EXTENSION)
    root = find_tree_group(bot, "logs_name")
    assert root is not None
    assert "logs_blacklistu_name" in get_subcommand_names(root)


async def test_subcommand_logs_blacklistr_name_registered():
    bot = await load_extension_bot(EXTENSION)
    root = find_tree_group(bot, "logs_name")
    assert root is not None
    assert "logs_blacklistr_name" in get_subcommand_names(root)
