...
class TestAddLevelXpCalculateXp:
    ...
    async def test_calculate_xp_returns_int(self) -> None:
        ...
        with (
            patch("minigames.addLevelXp.get_user_boost", new_callable=AsyncMock, return_value=None),
    ...
    async def test_calculate_xp_with_additive_user_boost(self) -> None:
        ...
        with (
            patch("minigames.addLevelXp.get_user_boost", new_callable=AsyncMock, return_value=boost),
            patch("minigames.addLevelXp.get_user_roles_boosts", new_callable=AsyncMock, return_value=[]),
            patch("minigames.addLevelXp.get_channel_boost", new_callable=AsyncMock, return_value=None),
        ):
            result = await calculate_xp(mock_message, "88888")
...
class TestAddLevelXpIsBlacklisted:
    ...
    async def test_not_blacklisted(self) -> None:
        ...
        with patch(
            "minigames.addLevelXp.get_blacklist",
