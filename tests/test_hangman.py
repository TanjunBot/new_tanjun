...
class TestHangmanSteps:
    ...
    def test_first_step_is_empty_gallows(self) -> None:
...
class TestGetGuessedLetters:
    def test_no_guesses(self) -> None:
        result = get_guessed_letters([], "hello")
        assert result == "_____"

    def test_single_correct_guess(self) -> None:
        result = get_guessed_letters(["h"], "hello")
        assert result == "h____"

    def test_multiple_correct_guesses(self) -> None:
        result = get_guessed_letters(["h", "e", "l"], "hello")
        assert result == "hell_"

    def test_all_letters_guessed(self) -> None:
        result = get_guessed_letters(["h", "e", "l", "o"], "hello")
        assert result == "hello"

    def test_wrong_guesses_only(self) -> None:
        result = get_guessed_letters(["x", "y", "z"], "hello")
        assert result == "_____"

    def test_full_word_guess(self) -> None:
    ...
    def test_word_with_spaces_no_guesses(self) -> None:
        result = get_guessed_letters([], "he llo")
        # Space should be preserved, letters hidden
        assert " " in result

    def test_duplicate_guesses(self) -> None:
    ...
    def test_repeated_letters_in_word(self) -> None:
        result = get_guessed_letters(["l"], "hello")
        assert result == "__ll_"

    def test_full_wrong_word_guess(self) -> None:
...
class TestWrongLetters:
    ...
    def test_max_wrong_guesses(self) -> None:
...
class TestGetGuessedLettersDeep:
    def test_word_all_same_letter(self) -> None:
        result = get_guessed_letters(["a"], "aaa")
        assert result == "aaa"

    def test_guess_sentinel_string(self) -> None:
    ...
    def test_no_guesses_single_char_word(self) -> None:
        result = get_guessed_letters([], "a")
        assert result == "_"


class TestWrongLettersDeep:
    def test_sentinel_string_not_counted(self) -> None:
...
class TestGetGuessedLettersDeepEdgeCases:
    ...
    def test_empty_guesses_list(self) -> None:
        result = get_guessed_letters([], "test")
        assert result == "____"


class TestWrongLettersDeepEdgeCases:
    def test_exactly_11_wrong_ends_game(self) -> None:
        """11 wrong single-letter guesses means game over."""
        guesses = list("bcdfgxyzqwk")
        assert wrong_letters(guesses, "hello") == 11

    def test_zero_wrong_for_perfect_game(self) -> None:
        guesses = list("helo")
        assert wrong_letters(guesses, "hello") == 0

    def test_vowels_against_consonant_word(self) -> None:
        assert wrong_letters(["a", "e", "i", "o", "u"], "crypt") == 5

    def test_consonants_against_vowel_word(self) -> None:
        assert wrong_letters(["b", "c", "d"], "aieou") == 3

    def test_mixed_length_guesses(self) -> None:
