...
class TestConfigVariables:
    ...
    def test_env_vars_are_strings_or_none(self):
        from config import token, applicationId, database_ip, database_password
...
class TestConfigModuleImport:
    ...
    def test_all_expected_attributes(self):
        import config
        expected_attrs = [
            "version", "token", "applicationId", "adminIds", "activity",
            "database_ip", "database_port", "database_password", "database_user",
            "database_schema", "giphyAPIKey", "GithubAuthToken", "ImgBBApiKey",
            "openAiKey", "bytebin_url", "bytebin_password", "bytebin_username",
            "brawlstarsToken", "twitchSecret", "twitchId", "prefix",
        ]
