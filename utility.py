...
GithubAuthToken: Optional[str] = "YOUR_GITHUB_TOKEN" # str
ImgBBApiKey: Optional[str] = "YOUR_IMGBB_API_KEY" # str
bytebin_password: Optional[str] = "YOUR_BYTEBIN_PASSWORD" # str
bytebin_url: Optional[str] = "YOUR_BYTEBIN_URL" # str
bytebin_username: Optional[str] = "YOUR_BYTEBIN_USERNAME" # str
tenorAPIKey: Optional[str] = "YOUR_TENOR_API_KEY" # str
tenorCKey: Optional[str] = "YOUR_TENOR_CLIENT_KEY" # str
...
class tanjunEmbed:
    ...
    def __init__(
        self,
        *,
        colour: Union[int, discord.Colour, None] = 0xCB33F5,  # Hex color
        color: Union[int, discord.Colour, None] = 0xCB33F5,  # Hex color
        title: Any | None = None,
    ...
    def __bool__(self) -> bool:
        return any(
            (
                self.title, self.url, self.description, self.colour,
                self.fields, self.timestamp, self.author, self.thumbnail,
                self.footer, self.image, self.provider, self.video,
            )
        )
    ...
    @colour.setter
    def colour(self, value: Union[int, discord.Colour, None]) -> None:
        if value is None:
            self._colour = None
    ...
    def set_footer(self, *, text: Any | None = None, icon_url: Any | None = None) -> Self:
        self._footer = {}
        if text is not None: self._footer["text"] = str(text)
        if icon_url is not None: self._footer["icon_url"] = str(icon_url)
        return self
    ...
    def set_image(self, *, url: Any | None) -> Self:
        if url is None: self._image = {}
        else: self._image = {"url": str(url)}
        return self
    ...
    def set_thumbnail(self, *, url: Any | None) -> Self:
        if url is None: self._thumbnail = {}
        else: self._thumbnail = {"url": str(url)}
        return self
    ...
    def set_author(self, *, name: Any, url: Any | None = None, icon_url: Any | None = None) -> Self:
        ...
        if url is not None: self._author["url"] = str(url)
        if icon_url is not None: self._author["icon_url"] = str(icon_url)
        return self
    ...
    def add_field(self, *, name: Any, value: Any, inline: bool = True) -> Self:
        ...
        if not hasattr(self, "_fields") or not isinstance(self._fields, list): self._fields = []
    ...
    def insert_field_at(self, index: int, *, name: Any, value: Any, inline: bool = True) -> Self:
        field: Dict[str, Any] = {"inline": inline, "name": str(name), "value": str(value)}
        if not hasattr(self, "_fields") or not isinstance(self._fields, list): self._fields = []
        if 0 <= index <= len(self._fields): self._fields.insert(index, field)
        elif index < 0: self._fields.insert(0, field)
        else: self._fields.append(field)
        return self
    ...
    def remove_field(self, index: int) -> Self:
        try:
            if hasattr(self, "_fields") and isinstance(self._fields, list): del self._fields[index]
    ...
    def set_field_at(self, index: int, *, name: Any, value: Any, inline: bool = True) -> Self:
        ...
        except IndexError: raise IndexError("field index out of range")
        return self
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self._colour is not None: result["color"] = self._colour.value
        else:
            result.pop("colour", None)
            result.pop("color", None)
        if self._timestamp is not None:
            ts = self._timestamp
            if ts.tzinfo: result["timestamp"] = ts.astimezone(tz=datetime.timezone.utc).isoformat()
            else: result["timestamp"] = ts.replace(tzinfo=datetime.timezone.utc).isoformat()
        else: result.pop("timestamp", None)
        ...
        if "fields" in result and result["fields"]: pass
        elif not getattr(self, "_fields", []): result.pop("fields", None)
        elif "_fields" in result : result["fields"] = result.pop("_fields")
        return result

class commandInfo:
    def __init__(
        self, user: discord.abc.User, channel: discord.abc.GuildChannel, guild: discord.Guild,
        command: discord.app_commands.Command[Any, ..., Any], locale: str, message: discord.Message,
        permissions: discord.Permissions, reply: ABCCoroutine[Any, Any, discord.Message], client: discord.Client,
    ):
...
class NumericStringParser:
    def __init__(self) -> None:
        ...
        self.push_uminus_sign_action: Callable[[ParseResults], None] = lambda pr: self.exprStack.append("unary -") if pr and pr[0] == "-" else None
        ...
        self.fn: Dict[str, Callable[..., Union[float, int]]] = {
            ...
            "log": math.log, "log10": math.log10, "log2": math.log2, "exp": math.exp, "abs": abs, "trunc": math.trunc, "round": round,
            "sgn": lambda a: (abs(a) > 1e-12 and cmp(a, 0)) or 0, "sqrt": math.sqrt, "factorial": math.factorial,
            "degrees": math.degrees, "radians": math.radians, "ceil": math.ceil, "floor": math.floor,
            "pi": lambda: math.pi, "e": lambda: math.e, "fac": math.factorial,
        }
    ...
    def _setup_parser(self) -> Forward:
        ...
        fnumber = Combine(Word("+-" + nums, nums) + PyParsingOptional(point + PyParsingOptional(Word(nums))) + PyParsingOptional(e_literal + Word("+-" + nums, nums)))
        ...
        atom = (PyParsingOptional(Literal("-")).setParseAction(self.push_uminus_sign_action) +
                (func_call | ident.setParseAction(self.push_first_action) | fnumber.setParseAction(self.push_first_action) | (lpar + expr + rpar)))
        factor = Forward()
    ...
    def evaluateStack(self, s: List[Any]) -> Union[float, int]:
        if not s: raise ValueError("Evaluation stack is empty.")
        op: Any = s.pop()
        if op == "unary -":
            ...
            func = self.fn[op]
            if op in ["pi", "e"]: return func()
            else:
                if not s: raise ValueError(f"Stack empty for argument to function {op}.")
                arg = self.evaluateStack(s)
                return func(arg)
        elif isinstance(op, (int, float)): return op
        elif isinstance(op, str):
            try: return float(op)
            except ValueError: raise Exception(f"Invalid token or identifier on stack: {op}")
        else: raise Exception(f"Unexpected item on stack: {op} of type {type(op)}")

    def eval(self, num_string: str, parseAll: bool = True) -> Union[float, int]:
        self.exprStack = []
        try: self.bnf.parseString(num_string, parseAll=parseAll)
        except Exception as e: raise ValueError(f"Error parsing expression '{num_string}': {e}")
        if not self.exprStack: raise ValueError(f"Expression '{num_string}' did not yield a calculable stack.")
        try: return self.evaluateStack(self.exprStack[:])
        except ValueError as e: raise ValueError(f"Error evaluating expression '{num_string}': {e}")
        except Exception as e: raise Exception(f"Unexpected error evaluating '{num_string}': {e}")
...
async def getGif(query: str, amount: int = 1, limit: int = 10) -> List[str]:
    async with aiohttp.ClientSession() as session:
        async def fetch(url: str) -> Optional[Dict[str, Any]]:
            async with session.get(url) as response:
                if response.status != 200:
                    ...
                    return None
                try: return await response.json() # type: ignore[no-any-return]
        ...
        if not tenorAPIKey or not tenorCKey:
            ...
            return []
        request_url = f"https://tenor.googleapis.com/v2/search?q={query}&key={tenorAPIKey}&client_key={tenorCKey}&limit={limit}"
        r = await fetch(request_url)
        if r is None or "results" not in r or not isinstance(r["results"], list): return []
            ...
            try:
                ...
                if media_formats and isinstance(media_formats, dict):
                    ...
                    if medium_gif and isinstance(medium_gif, dict):
                        ...
                        if isinstance(url_val, str): gifs_found.append(url_val)
...
def get_highest_exponent(polynomial: str) -> int:
    ...
    for coefficient_str, x_part, _, exponent_str in terms:
        if not coefficient_str and not x_part: continue
        if x_part:
            if exponent_str:
                try: highest_exponent = max(highest_exponent, int(exponent_str))
                except ValueError: pass
            elif "x" in x_part: highest_exponent = max(highest_exponent, 1)
    return highest_exponent

def checkIfHasPro(guildid: int) -> bool: return guildid != 0
def checkIfhasPlus(userid: int) -> bool: return userid != 0

def missingLocalization(locale: str) -> None:
    try:
        if not GithubAuthToken: print("GithubAuthToken not configured."); return
        g = Github(GithubAuthToken); repo = g.get_repo("TanjunBot/new_tanjun")
        label = repo.get_label("missing localization")
        repo.create_issue(title=f"Missing localization for {locale}", body=f"Missing localization for {locale}", labels=[label])
    except Exception as e: print(f"Error creating GitHub issue for missing localization '{locale}': {e}")
...
def addFeedback(content: str, author: str) -> None:
    ...
    except Exception as e: print(f"Error creating GitHub issue for feedback from '{author}': {e}")

LevelScalingFunc = Callable[[int], float]
LEVEL_SCALINGS: Dict[str, LevelScalingFunc] = {
    "easy": lambda lvl: 100 * lvl, "medium": lambda lvl: 100 * (lvl**1.5),
    "hard": lambda lvl: 100 * (lvl**2), "extreme": lambda lvl: 100 * (lvl**2.5),
}

OperatorFunc = Callable[[Any, Any], Any]
UnaryOperatorFunc = Callable[[Any], Any]
operators_map: Dict[Type[ast.AST], Union[OperatorFunc, UnaryOperatorFunc]] = { # Changed key type to Type[ast.AST]
    ast.Add: op.add, ast.Sub: op.sub, ast.Mult: op.mul, ast.Div: op.truediv,
    ast.Pow: op.pow, ast.BitXor: op.xor, ast.USub: op.neg, ast.Mod: op.mod,
}

def sqrt_n(x: float, n: float = 2.0) -> float:
    if n == 0: raise ValueError("The root degree n cannot be zero.")
    if x < 0 and n % 2 == 0: raise ValueError("Cannot compute even root of a negative number with real numbers.")
    return float(x ** (1.0 / n))

def log_n(x: float, base: float = math.e) -> float:
    if x <= 0: raise ValueError("Logarithm undefined for non-positive values.")
    if base <= 0 or base == 1: raise ValueError("Logarithm base must be positive and not equal to 1.")
    return math.log(x, base)

VariablesType = Dict[str, Union[float, int, complex, Callable[..., Any]]] # Added complex

def eval_expr(expr: str, variables: Optional[VariablesType] = None) -> Union[float, int, complex]:
    if variables is None: variables = {}
    local_vars: VariablesType = {
        "pi": math.pi, "e": math.e, "sin": math.sin, "cos": math.cos, "tan": math.tan,
        "asin": math.asin, "acos": math.acos, "atan": math.atan, "log": log_n,
        "log10": lambda x_val: log_n(x_val, 10), "log2": lambda x_val: log_n(x_val, 2),
        "ln": lambda x_val: log_n(x_val, math.e), "sqrt": sqrt_n, "nthroot": sqrt_n,
        "abs": abs, "floor": math.floor, "ceil": math.ceil, "pow": pow,
    }
    local_vars.update(variables)
    try:
        parsed_ast = ast.parse(expr, mode="eval").body
        return eval_(parsed_ast, local_vars)
    except SyntaxError as e: raise ValueError(f"Syntax error in expression: {expr}. Details: {e}")
    except TypeError as e: raise ValueError(f"Type error during evaluation of: {expr}. Details: {e}")
    except NameError as e: raise NameError(f"Name error in expression: {expr}. Details: {e}")
    except Exception as e: print(f"Unexpected error evaluating expression '{expr}': {type(e).__name__}: {e}"); raise

def eval_(node: ast.AST, variables: VariablesType) -> Union[float, int, complex]:
    if isinstance(node, ast.Constant):
        if isinstance(node.value, (int, float, complex)):
            return node.value
        # elif isinstance(node.value, bool): return int(node.value) # Optional: handle bools
        else: raise TypeError(f"Unsupported constant type in expression: {type(node.value)} ({node.value!r})")
        ...
        if isinstance(func_name_node, ast.Name):
            ...
            if func_id_str in variables:
                ...
                if callable(potential_func): func_to_call = potential_func
                else: raise TypeError(f"'{func_id_str}' is not a callable function (type: {type(potential_func)}).")
            else: raise NameError(f"Function '{func_id_str}' is not defined in variables.")
        if not func_to_call: raise TypeError(f"'{func_id_str}' is not a callable function.")
        args = [eval_(arg, variables) for arg in node.args]
        try: return func_to_call(*args) # type: ignore[no-any-return]
        except TypeError as e: raise TypeError(f"Error calling function '{getattr(func_to_call, '__name__', func_id_str)}': {e}")
    elif isinstance(node, ast.Name):
        if node.id in variables:
            val = variables[node.id]
            if callable(val): raise TypeError(f"Variable '{node.id}' holds a function but was not called. Use {node.id}(...).")
    ...
    else: raise TypeError(f"Unsupported AST node type: {type(node).__name__}")
...
def get_xp_for_level(level: int, scaling: str, custom_formula: Optional[str] = None) -> int:
    ...
    if scaling == "custom" and custom_formula:
        try: result = eval_expr(custom_formula, variables={"level": level})
        except (ValueError, NameError, TypeError, Exception) as e:
            print(f"Error evaluating custom XP formula '{custom_formula}' for level {level}: {e}"); return 0
    elif scaling in LEVEL_SCALINGS: result = LEVEL_SCALINGS[scaling](level)
    else: print(f"Warning: Unknown XP scaling '{scaling}'. Using 'medium'."); result = LEVEL_SCALINGS["medium"](level)
    ...
    if isinstance(result, complex):
        ...
        if math.isinf(result) or math.isnan(result): print(f"Warning: XP calculation resulted in {result}. Returning 0."); return 0
...
def get_level_for_xp(xp: int, scaling: str, custom_formula: Optional[str] = None) -> int:
    ...
    while search_low <= search_high:
        ...
        if mid == 0: search_low = 1; continue # Should not happen if low starts at 1
        xp_needed_for_mid = get_xp_for_level(mid, scaling, custom_formula)
        if xp_needed_for_mid <= xp: ans, search_low = mid, mid + 1
        else: search_high = mid - 1
    return ans

def relativeTimeStrToDate(time_string: str) -> datetime.datetime:
    if not time_string.strip(): return datetime.datetime.now(datetime.timezone.utc)
    pattern = r"(\d+)\s*([smhd])"; matches = re.findall(pattern, time_string.lower())
    if not matches: return datetime.datetime.now(datetime.timezone.utc)
    delta_kwargs: Dict[str, int] = {"days": 0, "hours": 0, "minutes": 0, "seconds": 0}
    for value_str, unit in matches:
        try:
            value = int(value_str)
            if unit == "s": delta_kwargs["seconds"] += value
            elif unit == "m": delta_kwargs["minutes"] += value
            elif unit == "h": delta_kwargs["hours"] += value
            elif unit == "d": delta_kwargs["days"] += value
        except ValueError: print(f"Warning: Invalid number '{value_str}' in time string '{time_string}'.")
    return datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(**delta_kwargs)

def relativeTimeToSeconds(time_string: str) -> int:
    if not time_string.strip(): return 0
    pattern = r"(\d+)\s*([smhd])"; matches = re.findall(pattern, time_string.lower())
...
def dateToRelativeTimeStr(date_obj: datetime.datetime) -> str:
    ...
    components: List[str] = []
    if days > 0: components.append(f"{days}d")
    if hours > 0: components.append(f"{hours}h")
    if minutes > 0: components.append(f"{minutes}m")
    if seconds > 0: components.append(f"{seconds}s")
    return " ".join(components) if components else "now"

class InteractionClient(Protocol):
    user: Optional[discord.ClientUser]; application_id: Optional[int]; _connection: Any
class InteractionGuild(Protocol): id: int; name: str
class InteractionChannel(Protocol): id: int
class InteractionUser(Protocol): id: int; name: str; discriminator: str; avatar: Optional[str]; nick: Optional[str]

class MockInteraction(discord.Interaction):
    def __init__(self, bot: InteractionClient, guild: Optional[InteractionGuild],
                 channel: Optional[InteractionChannel], user: InteractionUser):
        mock_snowflake_id = discord.utils.time_snowflake(datetime.datetime.now(datetime.timezone.utc))
        mock_data: Dict[str, Any] = {
            "id": mock_snowflake_id, "application_id": bot.application_id or 0,
            "type": discord.InteractionType.application_command.value, "data": {},
            "guild_id": guild.id if guild else None, "channel_id": channel.id if channel else None,
            "token": "mock_interaction_token_xxxxxxxxxxxx", "version": 1,
        }
        if guild and isinstance(user, discord.Member):
            ...
            mock_data["user"] = {"id": user.id, "username": user.name, "discriminator": user.discriminator, "avatar": user.avatar}
    ...
    @property
    def guild(self) -> Optional[discord.Guild]: return self._mock_guild 
    @property
    def channel(self) -> Optional[discord.abc.MessageableChannel]: return self._mock_channel 
    @property
    def user(self) -> Union[discord.User, discord.Member]: return self._mock_user
...
class MockInteractionResponse(discord.InteractionResponse): 
    ...
    async def send_message(self, content: Optional[str] = None, *, embed: Optional[tanjunEmbed] = None, **kwargs: Any) -> None: 
        if self.interaction._response_issued: raise discord.InteractionResponded(self.interaction) 
        message_data: Dict[str, Any] = {
            ...
            "content": content or "", "embeds": [embed.to_dict()] if embed else [],
            "author": {
            ...
            "timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat(), "edited_timestamp": None, "tts": False,
            "mention_everyone": False, "mentions": [], "mention_roles": [], "mention_channels": [],
            "attachments": [], "pinned": False, "type": discord.MessageType.default.value, "webhook_id": None,
        }
    ...
    async def edit_message(self, **kwargs: Any) -> None:
        ...
        if content is not None and self.message: self.message.content = content
        if "embed" in kwargs and self.message:
            self.message.embeds = [embed_val.to_dict()] if embed_val else []

class MockWebhook:
    def __init__(self, state: Any, application_id: Optional[int], token: str):
    ...
    async def send(self, content: Optional[str] = None, *, embed: Optional[tanjunEmbed] = None, **kwargs: Any) -> discord.Message:
        message_data: Dict[str, Any] = {
            "id": discord.utils.time_snowflake(datetime.datetime.now(datetime.timezone.utc)), "channel_id": 0,
            "content": content or "", "embeds": [embed.to_dict()] if embed else [],
            "author": {"id": self.application_id or 0, "username": "MockApp", "discriminator": "0000", "bot": True, "avatar": None},
            "timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat(), "webhook_id": str(self.id),
        }
        mock_channel = None
    ...
    async def edit_message(self, message_id: int, **kwargs: Any) -> discord.Message: raise NotImplementedError
    async def delete_message(self, message_id: int) -> None: pass
...
def create_mock_interaction(bot_instance: InteractionClient) -> MockInteraction:
    ...
    if not bot_instance or not bot_instance.user: raise ValueError("Bot instance or bot user is not valid.")
    mock_user_data = {'id': bot_instance.user.id, 'name': bot_instance.user.name,
                      'discriminator': bot_instance.user.discriminator, 'avatar': bot_instance.user.avatar,
...
async def upload_image_to_imgbb(image_bytes: bytes, file_extension: str) -> Optional[Dict[str, Any]]:
    clean_extension, temp_file_path = file_extension.lstrip("."), ""
    if not ImgBBApiKey: print("ImgBB API Key not configured."); return None
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix="." + clean_extension, mode="wb") as temp_f:
            temp_f.write(image_bytes); temp_file_path = temp_f.name
        async with aiohttp.ClientSession() as session:
            form_data = aiohttp.FormData()
            form_data.add_field("key", ImgBBApiKey)
            if not os.path.exists(temp_file_path): print(f"Error: Temp file {temp_file_path} vanished."); return None
            with open(temp_file_path, "rb") as f_up: form_data.add_field("image", f_up, filename=f"tbg.{clean_extension}")
            async with session.post("https://api.imgbb.com/1/upload", data=form_data) as response:
                if response.status == 200: return await response.json() # type: ignore[no-any-return]
                else: print(f"ImgBB upload failed. Status: {response.status}, Response: {await response.text()}"); return None
    except FileNotFoundError: print(f"Error: Temp file {temp_file_path} not found."); return None
    except Exception as e: print(f"An error occurred during ImgBB upload: {e}"); return None
    finally:
        if temp_file_path and os.path.exists(temp_file_path):
            try: os.remove(temp_file_path)
            except OSError as e: print(f"Error deleting temporary file {temp_file_path}: {e}")

async def upload_to_tanjun_logs(content: str) -> Optional[str]:
    if not all([bytebin_url, bytebin_username, bytebin_password]): print("Bytebin config missing."); return None
    try:
        compressed_content = gzip.compress(content.encode("utf-8"))
        url, username, password = str(bytebin_url), str(bytebin_username), str(bytebin_password)
        async with aiohttp.ClientSession() as session:
            auth, headers = aiohttp.BasicAuth(username, password), {"Content-Type": "application/octet-stream", "Content-Encoding": "gzip"}
            ...
            async with session.post(post_url, data=compressed_content, headers=headers, auth=auth) as response:
                if response.status == 201:
                    try:
                        ...
                        if "key" in resp_data and isinstance(resp_data["key"], str): return f"{url.rstrip('/')}/{resp_data['key']}"
                        print(f"Bytebin: 'key' not found/string in response: {resp_data}"); return None
                    except aiohttp.ContentTypeError:
                        key_text = await response.text()
                        if key_text: return f"{url.rstrip('/')}/{key_text.strip()}"
                        print(f"Bytebin: 201 but not JSON and no text key: {await response.text()}"); return None
                else: print(f"Bytebin upload failed. Status: {response.status}, Response: {await response.text()}"); return None
    except Exception as e: print(f"An error occurred during log upload to Bytebin: {e}"); return None

def check_if_str_is_hex_color(color_str: str) -> bool:
    processed_color_str = color_str.lstrip("#")
    if not all(c in "0123456789abcdefABCDEF" for c in processed_color_str): return False
    return len(processed_color_str) in (3, 6)

def draw_text_with_outline(
    draw: Any, position: Tuple[int, int], text: str, font: Any,
    text_color: Union[str, Tuple[int, int, int], Tuple[int, int, int, int]],
    outline_color: Union[str, Tuple[int, int, int], Tuple[int, int, int, int]],
) -> None:
    x, y = position; offset = 1
...
def isoTimeToDate(isoTime: str) -> datetime.datetime:
    try: return datetime.datetime.fromisoformat(isoTime)
    except ValueError as e: print(f"Error parsing ISO time string '{isoTime}': {e}"); raise

def similar(a: str, b: str) -> float:
    return SequenceMatcher(None, a, b).ratio()

def addThousandsSeparator(number: Union[int, float], separator: str = " ") -> str:
    if isinstance(number, (int, float)): return f"{number:,}".replace(",", separator)
